@font-face {
    font-family: 'wf_segoe-ui_light';
    src: url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/light/latest.eot');
    src: local("Segoe UI Light"), local("Segoe WP Light"), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/light/latest.eot?#iefix') format('embedded-opentype'), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/light/latest.woff') format('woff'), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/light/latest.ttf') format('truetype'), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/light/latest.svg#web') format('svg');
    font-weight: normal;
    font-style: normal
}
@font-face {
    font-family: 'wf_segoe-ui_normal';
    src: url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/normal/latest.eot');
    src: local("Segoe UI"), local("Segoe"), local("Segoe WP"), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/normal/latest.eot?#iefix') format('embedded-opentype'), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/normal/latest.woff') format('woff'), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/normal/latest.ttf') format('truetype'), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/normal/latest.svg#web') format('svg');
    font-weight: normal;
    font-style: normal
}
@font-face {
    font-family: 'wf_segoe-ui_Semibold';
    src: url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/Semibold/latest.eot');
    src: local("Segoe UI Semibold"), local("Segoe Semibold"), local("Segoe WP Semibold"), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/Semibold/latest.eot?#iefix') format('embedded-opentype'), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/Semibold/latest.woff') format('woff'), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/Semibold/latest.ttf') format('truetype'), url('//c.s-microsoft.com/static/fonts/segoe-ui/west-european/Semibold/latest.svg#web') format('svg');
    font-weight: normal;
    font-style: normal
}
@font-face {
    font-family: 'wf_icons_normal';
    src: url('//c.s-microsoft.com/static/fonts/icons/homepage/normal/2.eot');
    src: url('//c.s-microsoft.com/static/fonts/icons/homepage/normal/2.eot?#iefix') format('embedded-opentype'), url('//c.s-microsoft.com/static/fonts/icons/homepage/normal/2.woff') format('woff'), url('//c.s-microsoft.com/static/fonts/icons/homepage/normal/2.ttf') format('truetype'), url('//c.s-microsoft.com/static/fonts/icons/homepage/normal/2.svg#web') format('svg');
    font-weight: normal;
    font-style: normal
}
@-ms-viewport {
    width: device-width;
}
.CSPvNext html {
    overflow-y: auto
}
.CSPvNext .row-fluid {
    width: 100%;
    *zoom: 1
}
.CSPvNext .row-fluid.mscom-grid-container,
.CSPvNext .mscom-grid-container {
    width: 96%
}
.CSPvNext .row-fluid:before,
.CSPvNext .row-fluid:after {
    display: table;
    line-height: 0;
    content: ""
}
.CSPvNext .row-fluid:after {
    clear: both
}
.CSPvNext .row-fluid .span {
    display: block;
    float: left;
    width: 100%;
    margin-left: 2%;
    -webkit-box-sizing: border-box;
    -moz-box-sizing: border-box;
    box-sizing: border-box
}
.CSPvNext .row-fluid>.span:first-child {
    margin-left: 0
}
.CSPvNext .row-fluid>.bp3-clear,
.CSPvNext .row-fluid>.bp3-col-1-1 {
    margin-left: 0
}
.CSPvNext .row-fluid .bp3-col-1-0,
.CSPvNext .row-fluid .bp3-col-2-0,
.CSPvNext .row-fluid .bp3-col-3-0,
.CSPvNext .row-fluid .bp3-col-4-0,
.CSPvNext .row-fluid .bp3-col-5-0,
.CSPvNext .row-fluid .bp3-col-6-0,
.CSPvNext .row-fluid .bp3-col-7-0,
.CSPvNext .row-fluid .bp3-col-8-0,
.CSPvNext .row-fluid .bp3-col-9-0,
.CSPvNext .row-fluid .bp3-col-10-0,
.CSPvNext .row-fluid .bp3-col-11-0,
.CSPvNext .row-fluid .bp3-col-12-0 {
    display: none
}
.CSPvNext .row-fluid>.bp3-col-1,
.CSPvNext .row-fluid>.bp3-col-1-1,
.CSPvNext .row-fluid>.bp3-col-2-2,
.CSPvNext .row-fluid>.bp3-col-3-3,
.CSPvNext .row-fluid>.bp3-col-4-4,
.CSPvNext .row-fluid>.bp3-col-5-5,
.CSPvNext .row-fluid>.bp3-col-6-6,
.CSPvNext .row-fluid>.bp3-col-7-7,
.CSPvNext .row-fluid>.bp3-col-8-8,
.CSPvNext .row-fluid>.bp3-col-9-9,
.CSPvNext .row-fluid>.bp3-col-10-10,
.CSPvNext .row-fluid>.bp3-col-11-11,
.CSPvNext .row-fluid>.bp3-col-12-12 {
    width: 100%
}
.CSPvNext .row-fluid>.bp3-col-2,
.CSPvNext .row-fluid>.bp3-col-2-1,
.CSPvNext .row-fluid>.bp3-col-4-2,
.CSPvNext .row-fluid>.bp3-col-6-3,
.CSPvNext .row-fluid>.bp3-col-8-4,
.CSPvNext .row-fluid>.bp3-col-10-5,
.CSPvNext .row-fluid>.bp3-col-12-6 {
    width: 49%;
    *width: 48.9%
}
.CSPvNext .row-fluid>.bp3-col-3,
.CSPvNext .row-fluid>.bp3-col-3-1,
.CSPvNext .row-fluid>.bp3-col-6-2,
.CSPvNext .row-fluid>.bp3-col-9-3,
.CSPvNext .row-fluid>.bp3-col-12-4 {
    width: 32%;
    *width: 31.9%
}
.CSPvNext .row-fluid>.bp3-col-3-2,
.CSPvNext .row-fluid>.bp3-col-6-4,
.CSPvNext .row-fluid>.bp3-col-9-6,
.CSPvNext .row-fluid>.bp3-col-12-8 {
    width: 66%;
    *width: 65.9%
}
.CSPvNext .row-fluid>.bp3-col-4,
.CSPvNext .row-fluid>.bp3-col-4-1,
.CSPvNext .row-fluid>.bp3-col-8-2,
.CSPvNext .row-fluid>.bp3-col-12-3 {
    width: 23.5%;
    *width: 23.4%
}
.CSPvNext .row-fluid>.bp3-col-4-3,
.CSPvNext .row-fluid>.bp3-col-8-6 {
    width: 74.5%;
    *width: 74.4%
}
.CSPvNext .row-fluid>.bp3-col-5,
.CSPvNext .row-fluid>.bp3-col-5-1,
.CSPvNext .row-fluid>.bp3-col-10-2 {
    width: 18.4%;
    *width: 18.3%
}
.CSPvNext .row-fluid>.bp3-col-5-2 {
    width: 38.8%;
    *width: 38.7%
}
.CSPvNext .row-fluid>.bp3-col-5-3 {
    width: 59.2%;
    *width: 59.1%
}
.CSPvNext .row-fluid>.bp3-col-5-4 {
    width: 79.6%;
    *width: 79.5%
}
.CSPvNext .row-fluid>.bp3-col-6,
.CSPvNext .row-fluid>.bp3-col-6-1,
.CSPvNext .row-fluid>.bp3-col-12-2 {
    width: 15%;
    *width: 14.9%
}
.CSPvNext .row-fluid>.bp3-col-6-5 {
    width: 83%;
    *width: 82.9%
}
.CSPvNext .row-fluid>.bp3-col-7,
.CSPvNext .row-fluid>.bp3-col-7-1 {
    width: 12.57143%;
    *width: 12.47143%
}
.CSPvNext .row-fluid>.bp3-col-7-2 {
    width: 27.14286%;
    *width: 27.04286%
}
.CSPvNext .row-fluid>.bp3-col-7-3 {
    width: 41.71429%;
    *width: 41.61429%
}
.CSPvNext .row-fluid>.bp3-col-7-4 {
    width: 56.28571%;
    *width: 56.18571%
}
.CSPvNext .row-fluid>.bp3-col-7-5 {
    width: 70.85714%;
    *width: 70.75714%
}
.CSPvNext .row-fluid>.bp3-col-7-6 {
    width: 85.42857%;
    *width: 85.32857%
}
.CSPvNext .row-fluid>.bp3-col-8,
.CSPvNext .row-fluid>.bp3-col-8-1 {
    width: 10.75%;
    *width: 10.65%
}
.CSPvNext .row-fluid>.bp3-col-8-3 {
    width: 36.25%;
    *width: 36.15%
}
.CSPvNext .row-fluid>.bp3-col-8-5 {
    width: 61.75%;
    *width: 61.65%
}
.CSPvNext .row-fluid>.bp3-col-8-7 {
    width: 87.25%;
    *width: 87.15%
}
.CSPvNext .row-fluid>.bp3-col-9,
.CSPvNext .row-fluid>.bp3-col-9-1 {
    width: 9.33333%;
    *width: 9.23333%
}
.CSPvNext .row-fluid>.bp3-col-9-2 {
    width: 20.66667%;
    *width: 20.56667%
}
.CSPvNext .row-fluid>.bp3-col-9-4 {
    width: 43.33333%;
    *width: 43.23333%
}
.CSPvNext .row-fluid>.bp3-col-9-5 {
    width: 54.66667%;
    *width: 54.56667%
}
.CSPvNext .row-fluid>.bp3-col-9-7 {
    width: 77.33333%;
    *width: 77.23333%
}
.CSPvNext .row-fluid>.bp3-col-9-8 {
    width: 88.66667%;
    *width: 88.56667%
}
.CSPvNext .row-fluid>.bp3-col-10,
.CSPvNext .row-fluid>.bp3-col-10-1 {
    width: 8.2%;
    *width: 8.1%
}
.CSPvNext .row-fluid>.bp3-col-10-3 {
    width: 28.6%;
    *width: 28.5%
}
.CSPvNext .row-fluid>.bp3-col-10-4 {
    width: 38.8%;
    *width: 38.7%
}
.CSPvNext .row-fluid>.bp3-col-10-6 {
    width: 59.2%;
    *width: 59.1%
}
.CSPvNext .row-fluid>.bp3-col-10-7 {
    width: 69.4%;
    *width: 69.3%
}
.CSPvNext .row-fluid>.bp3-col-10-8 {
    width: 79.6%;
    *width: 79.5%
}
.CSPvNext .row-fluid>.bp3-col-10-9 {
    width: 89.8%;
    *width: 89.7%
}
.CSPvNext .row-fluid>.bp3-col-11,
.CSPvNext .row-fluid>.bp3-col-11-1 {
    width: 7.27273%;
    *width: 7.17273%
}
.CSPvNext .row-fluid>.bp3-col-11-2 {
    width: 16.54545%;
    *width: 16.44545%
}
.CSPvNext .row-fluid>.bp3-col-11-3 {
    width: 25.81818%;
    *width: 25.71818%
}
.CSPvNext .row-fluid>.bp3-col-11-4 {
    width: 35.09091%;
    *width: 34.99091%
}
.CSPvNext .row-fluid>.bp3-col-11-5 {
    width: 44.36364%;
    *width: 44.26364%
}
.CSPvNext .row-fluid>.bp3-col-11-6 {
    width: 53.63636%;
    *width: 53.53636%
}
.CSPvNext .row-fluid>.bp3-col-11-7 {
    width: 62.90909%;
    *width: 62.80909%
}
.CSPvNext .row-fluid>.bp3-col-11-8 {
    width: 72.18182%;
    *width: 72.08182%
}
.CSPvNext .row-fluid>.bp3-col-11-9 {
    width: 81.45455%;
    *width: 81.35455%
}
.CSPvNext .row-fluid>.bp3-col-11-10 {
    width: 90.72727%;
    *width: 90.62727%
}
.CSPvNext .row-fluid>.bp3-col-12,
.CSPvNext .row-fluid>.bp3-col-12-1 {
    width: 6.5%;
    *width: 6.4%
}
.CSPvNext .row-fluid>.bp3-col-12-5 {
    width: 40.5%;
    *width: 40.4%
}
.CSPvNext .row-fluid>.bp3-col-12-7 {
    width: 57.5%;
    *width: 57.4%
}
.CSPvNext .row-fluid>.bp3-col-12-9 {
    width: 74.5%;
    *width: 74.4%
}
.CSPvNext .row-fluid>.bp3-col-12-10 {
    width: 83%;
    *width: 82.9%
}
.CSPvNext .row-fluid>.bp3-col-12-11 {
    width: 91.5%;
    *width: 91.4%
}
@media screen and (max-width: 539px) {
    .CSPvNext .row-fluid>.span {
        width: 100%;
        margin-left: 0
    }
    .CSPvNext .row-fluid .bp0-clear {
        clear: both
    }
    .CSPvNext .row-fluid [class*="bp0-col"] {
        display: block
    }
    .CSPvNext .row-fluid>.bp0-clear,
    .CSPvNext .row-fluid>.bp0-col-1-1 {
        margin-left: 0
    }
    .CSPvNext .row-fluid .bp0-col-1-0,
    .CSPvNext .row-fluid .bp0-col-2-0,
    .CSPvNext .row-fluid .bp0-col-3-0,
    .CSPvNext .row-fluid .bp0-col-4-0,
    .CSPvNext .row-fluid .bp0-col-5-0,
    .CSPvNext .row-fluid .bp0-col-6-0,
    .CSPvNext .row-fluid .bp0-col-7-0,
    .CSPvNext .row-fluid .bp0-col-8-0,
    .CSPvNext .row-fluid .bp0-col-9-0,
    .CSPvNext .row-fluid .bp0-col-10-0,
    .CSPvNext .row-fluid .bp0-col-11-0,
    .CSPvNext .row-fluid .bp0-col-12-0 {
        display: none
    }
    .CSPvNext .row-fluid>.bp0-col-1,
    .CSPvNext .row-fluid>.bp0-col-1-1,
    .CSPvNext .row-fluid>.bp0-col-2-2,
    .CSPvNext .row-fluid>.bp0-col-3-3,
    .CSPvNext .row-fluid>.bp0-col-4-4,
    .CSPvNext .row-fluid>.bp0-col-5-5,
    .CSPvNext .row-fluid>.bp0-col-6-6,
    .CSPvNext .row-fluid>.bp0-col-7-7,
    .CSPvNext .row-fluid>.bp0-col-8-8,
    .CSPvNext .row-fluid>.bp0-col-9-9,
    .CSPvNext .row-fluid>.bp0-col-10-10,
    .CSPvNext .row-fluid>.bp0-col-11-11,
    .CSPvNext .row-fluid>.bp0-col-12-12 {
        width: 100%
    }
    .CSPvNext .row-fluid>.bp0-col-2,
    .CSPvNext .row-fluid>.bp0-col-2-1,
    .CSPvNext .row-fluid>.bp0-col-4-2,
    .CSPvNext .row-fluid>.bp0-col-6-3,
    .CSPvNext .row-fluid>.bp0-col-8-4,
    .CSPvNext .row-fluid>.bp0-col-10-5,
    .CSPvNext .row-fluid>.bp0-col-12-6 {
        width: 49%
    }
    .CSPvNext .row-fluid>.bp0-col-3,
    .CSPvNext .row-fluid>.bp0-col-3-1,
    .CSPvNext .row-fluid>.bp0-col-6-2,
    .CSPvNext .row-fluid>.bp0-col-9-3,
    .CSPvNext .row-fluid>.bp0-col-12-4 {
        width: 32%
    }
    .CSPvNext .row-fluid>.bp0-col-3-2,
    .CSPvNext .row-fluid>.bp0-col-6-4,
    .CSPvNext .row-fluid>.bp0-col-9-6,
    .CSPvNext .row-fluid>.bp0-col-12-8 {
        width: 66%
    }
    .CSPvNext .row-fluid>.bp0-col-4,
    .CSPvNext .row-fluid>.bp0-col-4-1,
    .CSPvNext .row-fluid>.bp0-col-8-2,
    .CSPvNext .row-fluid>.bp0-col-12-3 {
        width: 23.5%
    }
    .CSPvNext .row-fluid>.bp0-col-4-3,
    .CSPvNext .row-fluid>.bp0-col-8-6 {
        width: 74.5%
    }
    .CSPvNext .row-fluid>.bp0-col-5,
    .CSPvNext .row-fluid>.bp0-col-5-1,
    .CSPvNext .row-fluid>.bp0-col-10-2 {
        width: 18.4%
    }
    .CSPvNext .row-fluid>.bp0-col-5-2 {
        width: 38.8%
    }
    .CSPvNext .row-fluid>.bp0-col-5-3 {
        width: 59.2%
    }
    .CSPvNext .row-fluid>.bp0-col-5-4 {
        width: 79.6%
    }
    .CSPvNext .row-fluid>.bp0-col-6,
    .CSPvNext .row-fluid>.bp0-col-6-1,
    .CSPvNext .row-fluid>.bp0-col-12-2 {
        width: 15%
    }
    .CSPvNext .row-fluid>.bp0-col-6-5 {
        width: 83%
    }
    .CSPvNext .row-fluid>.bp0-col-7,
    .CSPvNext .row-fluid>.bp0-col-7-1 {
        width: 12.57143%
    }
    .CSPvNext .row-fluid>.bp0-col-7-2 {
        width: 27.14286%
    }
    .CSPvNext .row-fluid>.bp0-col-7-3 {
        width: 41.71429%
    }
    .CSPvNext .row-fluid>.bp0-col-7-4 {
        width: 56.28571%
    }
    .CSPvNext .row-fluid>.bp0-col-7-5 {
        width: 70.85714%
    }
    .CSPvNext .row-fluid>.bp0-col-7-6 {
        width: 85.42857%
    }
    .CSPvNext .row-fluid>.bp0-col-8,
    .CSPvNext .row-fluid>.bp0-col-8-1 {
        width: 10.75%
    }
    .CSPvNext .row-fluid>.bp0-col-8-3 {
        width: 36.25%
    }
    .CSPvNext .row-fluid>.bp0-col-8-5 {
        width: 61.75%
    }
    .CSPvNext .row-fluid>.bp0-col-8-7 {
        width: 87.25%
    }
    .CSPvNext .row-fluid>.bp0-col-9,
    .CSPvNext .row-fluid>.bp0-col-9-1 {
        width: 9.33333%
    }
    .CSPvNext .row-fluid>.bp0-col-9-2 {
        width: 20.66667%
    }
    .CSPvNext .row-fluid>.bp0-col-9-4 {
        width: 43.33333%
    }
    .CSPvNext .row-fluid>.bp0-col-9-5 {
        width: 54.66667%
    }
    .CSPvNext .row-fluid>.bp0-col-9-7 {
        width: 77.33333%
    }
    .CSPvNext .row-fluid>.bp0-col-9-8 {
        width: 88.66667%
    }
    .CSPvNext .row-fluid>.bp0-col-10,
    .CSPvNext .row-fluid>.bp0-col-10-1 {
        width: 8.2%
    }
    .CSPvNext .row-fluid>.bp0-col-10-3 {
        width: 28.6%
    }
    .CSPvNext .row-fluid>.bp0-col-10-4 {
        width: 38.8%
    }
    .CSPvNext .row-fluid>.bp0-col-10-6 {
        width: 59.2%
    }
    .CSPvNext .row-fluid>.bp0-col-10-7 {
        width: 69.4%
    }
    .CSPvNext .row-fluid>.bp0-col-10-8 {
        width: 79.6%
    }
    .CSPvNext .row-fluid>.bp0-col-10-9 {
        width: 89.8%
    }
    .CSPvNext .row-fluid>.bp0-col-11,
    .CSPvNext .row-fluid>.bp0-col-11-1 {
        width: 7.27273%
    }
    .CSPvNext .row-fluid>.bp0-col-11-2 {
        width: 16.54545%
    }
    .CSPvNext .row-fluid>.bp0-col-11-3 {
        width: 25.81818%
    }
    .CSPvNext .row-fluid>.bp0-col-11-4 {
        width: 35.09091%
    }
    .CSPvNext .row-fluid>.bp0-col-11-5 {
        width: 44.36364%
    }
    .CSPvNext .row-fluid>.bp0-col-11-6 {
        width: 53.63636%
    }
    .CSPvNext .row-fluid>.bp0-col-11-7 {
        width: 62.90909%
    }
    .CSPvNext .row-fluid>.bp0-col-11-8 {
        width: 72.18182%
    }
    .CSPvNext .row-fluid>.bp0-col-11-9 {
        width: 81.45455%
    }
    .CSPvNext .row-fluid>.bp0-col-11-10 {
        width: 90.72727%
    }
    .CSPvNext .row-fluid>.bp0-col-12,
    .CSPvNext .row-fluid>.bp0-col-12-1 {
        width: 6.5%
    }
    .CSPvNext .row-fluid>.bp0-col-12-5 {
        width: 40.5%
    }
    .CSPvNext .row-fluid>.bp0-col-12-7 {
        width: 57.5%
    }
    .CSPvNext .row-fluid>.bp0-col-12-9 {
        width: 74.5%
    }
    .CSPvNext .row-fluid>.bp0-col-12-10 {
        width: 83%
    }
    .CSPvNext .row-fluid>.bp0-col-12-11 {
        width: 91.5%
    }
}
@media screen and (min-width: 540px) and (max-width: 679px) {
    .CSPvNext .row-fluid .span {
        width: 100%
    }
    .CSPvNext .row-fluid .bp1-clear {
        clear: both
    }
    .CSPvNext .row-fluid [class*="bp1-col"] {
        display: block
    }
    .CSPvNext .row-fluid>.bp1-clear,
    .CSPvNext .row-fluid>.bp1-col-1-1 {
        margin-left: 0
    }
    .CSPvNext .row-fluid .bp1-col-1-0,
    .CSPvNext .row-fluid .bp1-col-2-0,
    .CSPvNext .row-fluid .bp1-col-3-0,
    .CSPvNext .row-fluid .bp1-col-4-0,
    .CSPvNext .row-fluid .bp1-col-5-0,
    .CSPvNext .row-fluid .bp1-col-6-0,
    .CSPvNext .row-fluid .bp1-col-7-0,
    .CSPvNext .row-fluid .bp1-col-8-0,
    .CSPvNext .row-fluid .bp1-col-9-0,
    .CSPvNext .row-fluid .bp1-col-10-0,
    .CSPvNext .row-fluid .bp1-col-11-0,
    .CSPvNext .row-fluid .bp1-col-12-0 {
        display: none
    }
    .CSPvNext .row-fluid>.bp1-col-1,
    .CSPvNext .row-fluid>.bp1-col-1-1,
    .CSPvNext .row-fluid>.bp1-col-2-2,
    .CSPvNext .row-fluid>.bp1-col-3-3,
    .CSPvNext .row-fluid>.bp1-col-4-4,
    .CSPvNext .row-fluid>.bp1-col-5-5,
    .CSPvNext .row-fluid>.bp1-col-6-6,
    .CSPvNext .row-fluid>.bp1-col-7-7,
    .CSPvNext .row-fluid>.bp1-col-8-8,
    .CSPvNext .row-fluid>.bp1-col-9-9,
    .CSPvNext .row-fluid>.bp1-col-10-10,
    .CSPvNext .row-fluid>.bp1-col-11-11,
    .CSPvNext .row-fluid>.bp1-col-12-12 {
        width: 100%
    }
    .CSPvNext .row-fluid>.bp1-col-2,
    .CSPvNext .row-fluid>.bp1-col-2-1,
    .CSPvNext .row-fluid>.bp1-col-4-2,
    .CSPvNext .row-fluid>.bp1-col-6-3,
    .CSPvNext .row-fluid>.bp1-col-8-4,
    .CSPvNext .row-fluid>.bp1-col-10-5,
    .CSPvNext .row-fluid>.bp1-col-12-6 {
        width: 49%
    }
    .CSPvNext .row-fluid>.bp1-col-3,
    .CSPvNext .row-fluid>.bp1-col-3-1,
    .CSPvNext .row-fluid>.bp1-col-6-2,
    .CSPvNext .row-fluid>.bp1-col-9-3,
    .CSPvNext .row-fluid>.bp1-col-12-4 {
        width: 32%
    }
    .CSPvNext .row-fluid>.bp1-col-3-2,
    .CSPvNext .row-fluid>.bp1-col-6-4,
    .CSPvNext .row-fluid>.bp1-col-9-6,
    .CSPvNext .row-fluid>.bp1-col-12-8 {
        width: 66%
    }
    .CSPvNext .row-fluid>.bp1-col-4,
    .CSPvNext .row-fluid>.bp1-col-4-1,
    .CSPvNext .row-fluid>.bp1-col-8-2,
    .CSPvNext .row-fluid>.bp1-col-12-3 {
        width: 23.5%
    }
    .CSPvNext .row-fluid>.bp1-col-4-3,
    .CSPvNext .row-fluid>.bp1-col-8-6 {
        width: 74.5%
    }
    .CSPvNext .row-fluid>.bp1-col-5,
    .CSPvNext .row-fluid>.bp1-col-5-1,
    .CSPvNext .row-fluid>.bp1-col-10-2 {
        width: 18.4%
    }
    .CSPvNext .row-fluid>.bp1-col-5-2 {
        width: 38.8%
    }
    .CSPvNext .row-fluid>.bp1-col-5-3 {
        width: 59.2%
    }
    .CSPvNext .row-fluid>.bp1-col-5-4 {
        width: 79.6%
    }
    .CSPvNext .row-fluid>.bp1-col-6,
    .CSPvNext .row-fluid>.bp1-col-6-1,
    .CSPvNext .row-fluid>.bp1-col-12-2 {
        width: 15%
    }
    .CSPvNext .row-fluid>.bp1-col-6-5 {
        width: 83%
    }
    .CSPvNext .row-fluid>.bp1-col-7,
    .CSPvNext .row-fluid>.bp1-col-7-1 {
        width: 12.57143%
    }
    .CSPvNext .row-fluid>.bp1-col-7-2 {
        width: 27.14286%
    }
    .CSPvNext .row-fluid>.bp1-col-7-3 {
        width: 41.71429%
    }
    .CSPvNext .row-fluid>.bp1-col-7-4 {
        width: 56.28571%
    }
    .CSPvNext .row-fluid>.bp1-col-7-5 {
        width: 70.85714%
    }
    .CSPvNext .row-fluid>.bp1-col-7-6 {
        width: 85.42857%
    }
    .CSPvNext .row-fluid>.bp1-col-8,
    .CSPvNext .row-fluid>.bp1-col-8-1 {
        width: 10.75%
    }
    .CSPvNext .row-fluid>.bp1-col-8-3 {
        width: 36.25%
    }
    .CSPvNext .row-fluid>.bp1-col-8-5 {
        width: 61.75%
    }
    .CSPvNext .row-fluid>.bp1-col-8-7 {
        width: 87.25%
    }
    .CSPvNext .row-fluid>.bp1-col-9,
    .CSPvNext .row-fluid>.bp1-col-9-1 {
        width: 9.33333%
    }
    .CSPvNext .row-fluid>.bp1-col-9-2 {
        width: 20.66667%
    }
    .CSPvNext .row-fluid>.bp1-col-9-4 {
        width: 43.33333%
    }
    .CSPvNext .row-fluid>.bp1-col-9-5 {
        width: 54.66667%
    }
    .CSPvNext .row-fluid>.bp1-col-9-7 {
        width: 77.33333%
    }
    .CSPvNext .row-fluid>.bp1-col-9-8 {
        width: 88.66667%
    }
    .CSPvNext .row-fluid>.bp1-col-10,
    .CSPvNext .row-fluid>.bp1-col-10-1 {
        width: 8.2%
    }
    .CSPvNext .row-fluid>.bp1-col-10-3 {
        width: 28.6%
    }
    .CSPvNext .row-fluid>.bp1-col-10-4 {
        width: 38.8%
    }
    .CSPvNext .row-fluid>.bp1-col-10-6 {
        width: 59.2%
    }
    .CSPvNext .row-fluid>.bp1-col-10-7 {
        width: 69.4%
    }
    .CSPvNext .row-fluid>.bp1-col-10-8 {
        width: 79.6%
    }
    .CSPvNext .row-fluid>.bp1-col-10-9 {
        width: 89.8%
    }
    .CSPvNext .row-fluid>.bp1-col-11,
    .CSPvNext .row-fluid>.bp1-col-11-1 {
        width: 7.27273%
    }
    .CSPvNext .row-fluid>.bp1-col-11-2 {
        width: 16.54545%
    }
    .CSPvNext .row-fluid>.bp1-col-11-3 {
        width: 25.81818%
    }
    .CSPvNext .row-fluid>.bp1-col-11-4 {
        width: 35.09091%
    }
    .CSPvNext .row-fluid>.bp1-col-11-5 {
        width: 44.36364%
    }
    .CSPvNext .row-fluid>.bp1-col-11-6 {
        width: 53.63636%
    }
    .CSPvNext .row-fluid>.bp1-col-11-7 {
        width: 62.90909%
    }
    .CSPvNext .row-fluid>.bp1-col-11-8 {
        width: 72.18182%
    }
    .CSPvNext .row-fluid>.bp1-col-11-9 {
        width: 81.45455%
    }
    .CSPvNext .row-fluid>.bp1-col-11-10 {
        width: 90.72727%
    }
    .CSPvNext .row-fluid>.bp1-col-12,
    .CSPvNext .row-fluid>.bp1-col-12-1 {
        width: 6.5%
    }
    .CSPvNext .row-fluid>.bp1-col-12-5 {
        width: 40.5%
    }
    .CSPvNext .row-fluid>.bp1-col-12-7 {
        width: 57.5%
    }
    .CSPvNext .row-fluid>.bp1-col-12-9 {
        width: 74.5%
    }
    .CSPvNext .row-fluid>.bp1-col-12-10 {
        width: 83%
    }
    .CSPvNext .row-fluid>.bp1-col-12-11 {
        width: 91.5%
    }
}
@media screen and (min-width: 680px) and (max-width: 899px) {
    .CSPvNext .row-fluid .span {
        width: 100%
    }
    .CSPvNext .row-fluid>.span {
        width: 100%
    }
    .CSPvNext .row-fluid .bp2-clear {
        clear: both
    }
    .CSPvNext .row-fluid [class*="bp2-col"] {
        display: block
    }
    .CSPvNext .row-fluid>.bp2-clear,
    .CSPvNext .row-fluid>.bp2-col-1-1 {
        margin-left: 0
    }
    .CSPvNext .row-fluid .bp2-col-1-0,
    .CSPvNext .row-fluid .bp2-col-2-0,
    .CSPvNext .row-fluid .bp2-col-3-0,
    .CSPvNext .row-fluid .bp2-col-4-0,
    .CSPvNext .row-fluid .bp2-col-5-0,
    .CSPvNext .row-fluid .bp2-col-6-0,
    .CSPvNext .row-fluid .bp2-col-7-0,
    .CSPvNext .row-fluid .bp2-col-8-0,
    .CSPvNext .row-fluid .bp2-col-9-0,
    .CSPvNext .row-fluid .bp2-col-10-0,
    .CSPvNext .row-fluid .bp2-col-11-0,
    .CSPvNext .row-fluid .bp2-col-12-0 {
        display: none
    }
    .CSPvNext .row-fluid>.bp2-col-1,
    .CSPvNext .row-fluid>.bp2-col-1-1,
    .CSPvNext .row-fluid>.bp2-col-2-2,
    .CSPvNext .row-fluid>.bp2-col-3-3,
    .CSPvNext .row-fluid>.bp2-col-4-4,
    .CSPvNext .row-fluid>.bp2-col-5-5,
    .CSPvNext .row-fluid>.bp2-col-6-6,
    .CSPvNext .row-fluid>.bp2-col-7-7,
    .CSPvNext .row-fluid>.bp2-col-8-8,
    .CSPvNext .row-fluid>.bp2-col-9-9,
    .CSPvNext .row-fluid>.bp2-col-10-10,
    .CSPvNext .row-fluid>.bp2-col-11-11,
    .CSPvNext .row-fluid>.bp2-col-12-12 {
        width: 100%
    }
    .CSPvNext .row-fluid>.bp2-col-2,
    .CSPvNext .row-fluid>.bp2-col-2-1,
    .CSPvNext .row-fluid>.bp2-col-4-2,
    .CSPvNext .row-fluid>.bp2-col-6-3,
    .CSPvNext .row-fluid>.bp2-col-8-4,
    .CSPvNext .row-fluid>.bp2-col-10-5,
    .CSPvNext .row-fluid>.bp2-col-12-6 {
        width: 49%
    }
    .CSPvNext .row-fluid>.bp2-col-3,
    .CSPvNext .row-fluid>.bp2-col-3-1,
    .CSPvNext .row-fluid>.bp2-col-6-2,
    .CSPvNext .row-fluid>.bp2-col-9-3,
    .CSPvNext .row-fluid>.bp2-col-12-4 {
        width: 32%
    }
    .CSPvNext .row-fluid>.bp2-col-3-2,
    .CSPvNext .row-fluid>.bp2-col-6-4,
    .CSPvNext .row-fluid>.bp2-col-9-6,
    .CSPvNext .row-fluid>.bp2-col-12-8 {
        width: 66%
    }
    .CSPvNext .row-fluid>.bp2-col-4,
    .CSPvNext .row-fluid>.bp2-col-4-1,
    .CSPvNext .row-fluid>.bp2-col-8-2,
    .CSPvNext .row-fluid>.bp2-col-12-3 {
        width: 23.5%
    }
    .CSPvNext .row-fluid>.bp2-col-4-3,
    .CSPvNext .row-fluid>.bp2-col-8-6 {
        width: 74.5%
    }
    .CSPvNext .row-fluid>.bp2-col-5,
    .CSPvNext .row-fluid>.bp2-col-5-1,
    .CSPvNext .row-fluid>.bp2-col-10-2 {
        width: 18.4%
    }
    .CSPvNext .row-fluid>.bp2-col-5-2 {
        width: 38.8%
    }
    .CSPvNext .row-fluid>.bp2-col-5-3 {
        width: 59.2%
    }
    .CSPvNext .row-fluid>.bp2-col-5-4 {
        width: 79.6%
    }
    .CSPvNext .row-fluid>.bp2-col-6,
    .CSPvNext .row-fluid>.bp2-col-6-1,
    .CSPvNext .row-fluid>.bp2-col-12-2 {
        width: 15%
    }
    .CSPvNext .row-fluid>.bp2-col-6-5 {
        width: 83%
    }
    .CSPvNext .row-fluid>.bp2-col-7,
    .CSPvNext .row-fluid>.bp2-col-7-1 {
        width: 12.57143%
    }
    .CSPvNext .row-fluid>.bp2-col-7-2 {
        width: 27.14286%
    }
    .CSPvNext .row-fluid>.bp2-col-7-3 {
        width: 41.71429%
    }
    .CSPvNext .row-fluid>.bp2-col-7-4 {
        width: 56.28571%
    }
    .CSPvNext .row-fluid>.bp2-col-7-5 {
        width: 70.85714%
    }
    .CSPvNext .row-fluid>.bp2-col-7-6 {
        width: 85.42857%
    }
    .CSPvNext .row-fluid>.bp2-col-8,
    .CSPvNext .row-fluid>.bp2-col-8-1 {
        width: 10.75%
    }
    .CSPvNext .row-fluid>.bp2-col-8-3 {
        width: 36.25%
    }
    .CSPvNext .row-fluid>.bp2-col-8-5 {
        width: 61.75%
    }
    .CSPvNext .row-fluid>.bp2-col-8-7 {
        width: 87.25%
    }
    .CSPvNext .row-fluid>.bp2-col-9,
    .CSPvNext .row-fluid>.bp2-col-9-1 {
        width: 9.33333%
    }
    .CSPvNext .row-fluid>.bp2-col-9-2 {
        width: 20.66667%
    }
    .CSPvNext .row-fluid>.bp2-col-9-4 {
        width: 43.33333%
    }
    .CSPvNext .row-fluid>.bp2-col-9-5 {
        width: 54.66667%
    }
    .CSPvNext .row-fluid>.bp2-col-9-7 {
        width: 77.33333%
    }
    .CSPvNext .row-fluid>.bp2-col-9-8 {
        width: 88.66667%
    }
    .CSPvNext .row-fluid>.bp2-col-10,
    .CSPvNext .row-fluid>.bp2-col-10-1 {
        width: 8.2%
    }
    .CSPvNext .row-fluid>.bp2-col-10-3 {
        width: 28.6%
    }
    .CSPvNext .row-fluid>.bp2-col-10-4 {
        width: 38.8%
    }
    .CSPvNext .row-fluid>.bp2-col-10-6 {
        width: 59.2%
    }
    .CSPvNext .row-fluid>.bp2-col-10-7 {
        width: 69.4%
    }
    .CSPvNext .row-fluid>.bp2-col-10-8 {
        width: 79.6%
    }
    .CSPvNext .row-fluid>.bp2-col-10-9 {
        width: 89.8%
    }
    .CSPvNext .row-fluid>.bp2-col-11,
    .CSPvNext .row-fluid>.bp2-col-11-1 {
        width: 7.27273%
    }
    .CSPvNext .row-fluid>.bp2-col-11-2 {
        width: 16.54545%
    }
    .CSPvNext .row-fluid>.bp2-col-11-3 {
        width: 25.81818%
    }
    .CSPvNext .row-fluid>.bp2-col-11-4 {
        width: 35.09091%
    }
    .CSPvNext .row-fluid>.bp2-col-11-5 {
        width: 44.36364%
    }
    .CSPvNext .row-fluid>.bp2-col-11-6 {
        width: 53.63636%
    }
    .CSPvNext .row-fluid>.bp2-col-11-7 {
        width: 62.90909%
    }
    .CSPvNext .row-fluid>.bp2-col-11-8 {
        width: 72.18182%
    }
    .CSPvNext .row-fluid>.bp2-col-11-9 {
        width: 81.45455%
    }
    .CSPvNext .row-fluid>.bp2-col-11-10 {
        width: 90.72727%
    }
    .CSPvNext .row-fluid>.bp2-col-12,
    .CSPvNext .row-fluid>.bp2-col-12-1 {
        width: 6.5%
    }
    .CSPvNext .row-fluid>.bp2-col-12-5 {
        width: 40.5%
    }
    .CSPvNext .row-fluid>.bp2-col-12-7 {
        width: 57.5%
    }
    .CSPvNext .row-fluid>.bp2-col-12-9 {
        width: 74.5%
    }
    .CSPvNext .row-fluid>.bp2-col-12-10 {
        width: 83%
    }
    .CSPvNext .row-fluid>.bp2-col-12-11 {
        width: 91.5%
    }
}
.CSPvNext .margin-row-fluid {
    width: 100%;
    *zoom: 1
}
.CSPvNext .margin-row-fluid.mscom-grid-container,
.CSPvNext .mscom-grid-container {
    width: 96%
}
.CSPvNext .margin-row-fluid:before,
.CSPvNext .margin-row-fluid:after {
    display: table;
    line-height: 0;
    content: ""
}
.CSPvNext .margin-row-fluid:after {
    clear: both
}
.CSPvNext .margin-row-fluid .span {
    display: block;
    float: left;
    width: 96%;
    margin-left: 2%;
    -webkit-box-sizing: border-box;
    -moz-box-sizing: border-box;
    box-sizing: border-box
}
.CSPvNext .margin-row-fluid>.span:first-child {
    margin-left: 2%
}
.CSPvNext .margin-row-fluid>.bp3-clear,
.CSPvNext .margin-row-fluid>.bp3-col-1-1 {
    margin-left: 2%
}
.CSPvNext .margin-row-fluid .bp3-col-1-0,
.CSPvNext .margin-row-fluid .bp3-col-2-0,
.CSPvNext .margin-row-fluid .bp3-col-3-0,
.CSPvNext .margin-row-fluid .bp3-col-4-0,
.CSPvNext .margin-row-fluid .bp3-col-5-0,
.CSPvNext .margin-row-fluid .bp3-col-6-0,
.CSPvNext .margin-row-fluid .bp3-col-7-0,
.CSPvNext .margin-row-fluid .bp3-col-8-0,
.CSPvNext .margin-row-fluid .bp3-col-9-0,
.CSPvNext .margin-row-fluid .bp3-col-10-0,
.CSPvNext .margin-row-fluid .bp3-col-11-0,
.CSPvNext .margin-row-fluid .bp3-col-12-0 {
    display: none
}
.CSPvNext .margin-row-fluid>.bp3-col-1,
.CSPvNext .margin-row-fluid>.bp3-col-1-1,
.CSPvNext .margin-row-fluid>.bp3-col-2-2,
.CSPvNext .margin-row-fluid>.bp3-col-3-3,
.CSPvNext .margin-row-fluid>.bp3-col-4-4,
.CSPvNext .margin-row-fluid>.bp3-col-5-5,
.CSPvNext .margin-row-fluid>.bp3-col-6-6,
.CSPvNext .margin-row-fluid>.bp3-col-7-7,
.CSPvNext .margin-row-fluid>.bp3-col-8-8,
.CSPvNext .margin-row-fluid>.bp3-col-9-9,
.CSPvNext .margin-row-fluid>.bp3-col-10-10,
.CSPvNext .margin-row-fluid>.bp3-col-11-11,
.CSPvNext .margin-row-fluid>.bp3-col-12-12 {
    width: 96%
}
.CSPvNext .margin-row-fluid>.bp3-col-2,
.CSPvNext .margin-row-fluid>.bp3-col-2-1,
.CSPvNext .margin-row-fluid>.bp3-col-4-2,
.CSPvNext .margin-row-fluid>.bp3-col-6-3,
.CSPvNext .margin-row-fluid>.bp3-col-8-4,
.CSPvNext .margin-row-fluid>.bp3-col-10-5,
.CSPvNext .margin-row-fluid>.bp3-col-12-6 {
    width: 47%;
    *width: 46.9%
}
.CSPvNext .margin-row-fluid>.bp3-col-3,
.CSPvNext .margin-row-fluid>.bp3-col-3-1,
.CSPvNext .margin-row-fluid>.bp3-col-6-2,
.CSPvNext .margin-row-fluid>.bp3-col-9-3,
.CSPvNext .margin-row-fluid>.bp3-col-12-4 {
    width: 30.66667%;
    *width: 30.56667%
}
.CSPvNext .margin-row-fluid>.bp3-col-3-2,
.CSPvNext .margin-row-fluid>.bp3-col-6-4,
.CSPvNext .margin-row-fluid>.bp3-col-9-6,
.CSPvNext .margin-row-fluid>.bp3-col-12-8 {
    width: 63.33333%;
    *width: 63.23333%
}
.CSPvNext .margin-row-fluid>.bp3-col-4,
.CSPvNext .margin-row-fluid>.bp3-col-4-1,
.CSPvNext .margin-row-fluid>.bp3-col-8-2,
.CSPvNext .margin-row-fluid>.bp3-col-12-3 {
    width: 22.5%;
    *width: 22.4%
}
.CSPvNext .margin-row-fluid>.bp3-col-4-3,
.CSPvNext .margin-row-fluid>.bp3-col-8-6 {
    width: 71.5%;
    *width: 71.4%
}
.CSPvNext .margin-row-fluid>.bp3-col-5,
.CSPvNext .margin-row-fluid>.bp3-col-5-1,
.CSPvNext .margin-row-fluid>.bp3-col-10-2 {
    width: 17.6%;
    *width: 17.5%
}
.CSPvNext .margin-row-fluid>.bp3-col-5-2 {
    width: 37.2%;
    *width: 37.1%
}
.CSPvNext .margin-row-fluid>.bp3-col-5-3 {
    width: 56.8%;
    *width: 56.7%
}
.CSPvNext .margin-row-fluid>.bp3-col-5-4 {
    width: 76.4%;
    *width: 76.3%
}
.CSPvNext .margin-row-fluid>.bp3-col-6,
.CSPvNext .margin-row-fluid>.bp3-col-6-1,
.CSPvNext .margin-row-fluid>.bp3-col-12-2 {
    width: 14.33333%;
    *width: 14.23333%
}
.CSPvNext .margin-row-fluid>.bp3-col-6-5 {
    width: 79.66667%;
    *width: 79.56667%
}
.CSPvNext .margin-row-fluid>.bp3-col-7,
.CSPvNext .margin-row-fluid>.bp3-col-7-1 {
    width: 12%;
    *width: 11.9%
}
.CSPvNext .margin-row-fluid>.bp3-col-7-2 {
    width: 26%;
    *width: 25.9%
}
.CSPvNext .margin-row-fluid>.bp3-col-7-3 {
    width: 40%;
    *width: 39.9%
}
.CSPvNext .margin-row-fluid>.bp3-col-7-4 {
    width: 54%;
    *width: 53.9%
}
.CSPvNext .margin-row-fluid>.bp3-col-7-5 {
    width: 68%;
    *width: 67.9%
}
.CSPvNext .margin-row-fluid>.bp3-col-7-6 {
    width: 82%;
    *width: 81.9%
}
.CSPvNext .margin-row-fluid>.bp3-col-8,
.CSPvNext .margin-row-fluid>.bp3-col-8-1 {
    width: 10.25%;
    *width: 10.15%
}
.CSPvNext .margin-row-fluid>.bp3-col-8-3 {
    width: 34.75%;
    *width: 34.65%
}
.CSPvNext .margin-row-fluid>.bp3-col-8-5 {
    width: 59.25%;
    *width: 59.15%
}
.CSPvNext .margin-row-fluid>.bp3-col-8-7 {
    width: 83.75%;
    *width: 83.65%
}
.CSPvNext .margin-row-fluid>.bp3-col-9,
.CSPvNext .margin-row-fluid>.bp3-col-9-1 {
    width: 8.88889%;
    *width: 8.78889%
}
.CSPvNext .margin-row-fluid>.bp3-col-9-2 {
    width: 19.77778%;
    *width: 19.67778%
}
.CSPvNext .margin-row-fluid>.bp3-col-9-4 {
    width: 41.55556%;
    *width: 41.45556%
}
.CSPvNext .margin-row-fluid>.bp3-col-9-5 {
    width: 52.44444%;
    *width: 52.34444%
}
.CSPvNext .margin-row-fluid>.bp3-col-9-7 {
    width: 74.22222%;
    *width: 74.12222%
}
.CSPvNext .margin-row-fluid>.bp3-col-9-8 {
    width: 85.11111%;
    *width: 85.01111%
}
.CSPvNext .margin-row-fluid>.bp3-col-10,
.CSPvNext .margin-row-fluid>.bp3-col-10-1 {
    width: 7.8%;
    *width: 7.7%
}
.CSPvNext .margin-row-fluid>.bp3-col-10-3 {
    width: 27.4%;
    *width: 27.3%
}
.CSPvNext .margin-row-fluid>.bp3-col-10-4 {
    width: 37.2%;
    *width: 37.1%
}
.CSPvNext .margin-row-fluid>.bp3-col-10-6 {
    width: 56.8%;
    *width: 56.7%
}
.CSPvNext .margin-row-fluid>.bp3-col-10-7 {
    width: 66.6%;
    *width: 66.5%
}
.CSPvNext .margin-row-fluid>.bp3-col-10-8 {
    width: 76.4%;
    *width: 76.3%
}
.CSPvNext .margin-row-fluid>.bp3-col-10-9 {
    width: 86.2%;
    *width: 86.1%
}
.CSPvNext .margin-row-fluid>.bp3-col-11,
.CSPvNext .margin-row-fluid>.bp3-col-11-1 {
    width: 6.90909%;
    *width: 6.80909%
}
.CSPvNext .margin-row-fluid>.bp3-col-11-2 {
    width: 15.81818%;
    *width: 15.71818%
}
.CSPvNext .margin-row-fluid>.bp3-col-11-3 {
    width: 24.72727%;
    *width: 24.62727%
}
.CSPvNext .margin-row-fluid>.bp3-col-11-4 {
    width: 33.63636%;
    *width: 33.53636%
}
.CSPvNext .margin-row-fluid>.bp3-col-11-5 {
    width: 42.54545%;
    *width: 42.44545%
}
.CSPvNext .margin-row-fluid>.bp3-col-11-6 {
    width: 51.45455%;
    *width: 51.35455%
}
.CSPvNext .margin-row-fluid>.bp3-col-11-7 {
    width: 60.36364%;
    *width: 60.26364%
}
.CSPvNext .margin-row-fluid>.bp3-col-11-8 {
    width: 69.27273%;
    *width: 69.17273%
}
.CSPvNext .margin-row-fluid>.bp3-col-11-9 {
    width: 78.18182%;
    *width: 78.08182%
}
.CSPvNext .margin-row-fluid>.bp3-col-11-10 {
    width: 87.09091%;
    *width: 86.99091%
}
.CSPvNext .margin-row-fluid>.bp3-col-12,
.CSPvNext .margin-row-fluid>.bp3-col-12-1 {
    width: 6.16667%;
    *width: 6.06667%
}
.CSPvNext .margin-row-fluid>.bp3-col-12-5 {
    width: 38.83333%;
    *width: 38.73333%
}
.CSPvNext .margin-row-fluid>.bp3-col-12-7 {
    width: 55.16667%;
    *width: 55.06667%
}
.CSPvNext .margin-row-fluid>.bp3-col-12-9 {
    width: 71.5%;
    *width: 71.4%
}
.CSPvNext .margin-row-fluid>.bp3-col-12-10 {
    width: 79.66667%;
    *width: 79.56667%
}
.CSPvNext .margin-row-fluid>.bp3-col-12-11 {
    width: 87.83333%;
    *width: 87.73333%
}
@media screen and (max-width: 539px) {
    .CSPvNext .margin-row-fluid>.span {
        width: 96%
    }
    .CSPvNext .margin-row-fluid .bp0-clear {
        clear: both
    }
    .CSPvNext .margin-row-fluid [class*="bp0-col"] {
        display: block
    }
    .CSPvNext .margin-row-fluid>.bp0-clear,
    .CSPvNext .margin-row-fluid>.bp0-col-1-1 {
        margin-left: 2%
    }
    .CSPvNext .margin-row-fluid .bp0-col-1-0,
    .CSPvNext .margin-row-fluid .bp0-col-2-0,
    .CSPvNext .margin-row-fluid .bp0-col-3-0,
    .CSPvNext .margin-row-fluid .bp0-col-4-0,
    .CSPvNext .margin-row-fluid .bp0-col-5-0,
    .CSPvNext .margin-row-fluid .bp0-col-6-0,
    .CSPvNext .margin-row-fluid .bp0-col-7-0,
    .CSPvNext .margin-row-fluid .bp0-col-8-0,
    .CSPvNext .margin-row-fluid .bp0-col-9-0,
    .CSPvNext .margin-row-fluid .bp0-col-10-0,
    .CSPvNext .margin-row-fluid .bp0-col-11-0,
    .CSPvNext .margin-row-fluid .bp0-col-12-0 {
        display: none
    }
    .CSPvNext .margin-row-fluid>.bp0-col-1,
    .CSPvNext .margin-row-fluid>.bp0-col-1-1,
    .CSPvNext .margin-row-fluid>.bp0-col-2-2,
    .CSPvNext .margin-row-fluid>.bp0-col-3-3,
    .CSPvNext .margin-row-fluid>.bp0-col-4-4,
    .CSPvNext .margin-row-fluid>.bp0-col-5-5,
    .CSPvNext .margin-row-fluid>.bp0-col-6-6,
    .CSPvNext .margin-row-fluid>.bp0-col-7-7,
    .CSPvNext .margin-row-fluid>.bp0-col-8-8,
    .CSPvNext .margin-row-fluid>.bp0-col-9-9,
    .CSPvNext .margin-row-fluid>.bp0-col-10-10,
    .CSPvNext .margin-row-fluid>.bp0-col-11-11,
    .CSPvNext .margin-row-fluid>.bp0-col-12-12 {
        width: 96%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-2,
    .CSPvNext .margin-row-fluid>.bp0-col-2-1,
    .CSPvNext .margin-row-fluid>.bp0-col-4-2,
    .CSPvNext .margin-row-fluid>.bp0-col-6-3,
    .CSPvNext .margin-row-fluid>.bp0-col-8-4,
    .CSPvNext .margin-row-fluid>.bp0-col-10-5,
    .CSPvNext .margin-row-fluid>.bp0-col-12-6 {
        width: 47%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-3,
    .CSPvNext .margin-row-fluid>.bp0-col-3-1,
    .CSPvNext .margin-row-fluid>.bp0-col-6-2,
    .CSPvNext .margin-row-fluid>.bp0-col-9-3,
    .CSPvNext .margin-row-fluid>.bp0-col-12-4 {
        width: 30.66667%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-3-2,
    .CSPvNext .margin-row-fluid>.bp0-col-6-4,
    .CSPvNext .margin-row-fluid>.bp0-col-9-6,
    .CSPvNext .margin-row-fluid>.bp0-col-12-8 {
        width: 63.33333%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-4,
    .CSPvNext .margin-row-fluid>.bp0-col-4-1,
    .CSPvNext .margin-row-fluid>.bp0-col-8-2,
    .CSPvNext .margin-row-fluid>.bp0-col-12-3 {
        width: 22.5%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-4-3,
    .CSPvNext .margin-row-fluid>.bp0-col-8-6 {
        width: 71.5%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-5,
    .CSPvNext .margin-row-fluid>.bp0-col-5-1,
    .CSPvNext .margin-row-fluid>.bp0-col-10-2 {
        width: 17.6%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-5-2 {
        width: 37.2%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-5-3 {
        width: 56.8%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-5-4 {
        width: 76.4%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-6,
    .CSPvNext .margin-row-fluid>.bp0-col-6-1,
    .CSPvNext .margin-row-fluid>.bp0-col-12-2 {
        width: 14.33333%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-6-5 {
        width: 79.66667%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-7,
    .CSPvNext .margin-row-fluid>.bp0-col-7-1 {
        width: 12%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-7-2 {
        width: 26%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-7-3 {
        width: 40%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-7-4 {
        width: 54%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-7-5 {
        width: 68%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-7-6 {
        width: 82%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-8,
    .CSPvNext .margin-row-fluid>.bp0-col-8-1 {
        width: 10.25%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-8-3 {
        width: 34.75%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-8-5 {
        width: 59.25%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-8-7 {
        width: 83.75%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-9,
    .CSPvNext .margin-row-fluid>.bp0-col-9-1 {
        width: 8.88889%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-9-2 {
        width: 19.77778%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-9-4 {
        width: 41.55556%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-9-5 {
        width: 52.44444%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-9-7 {
        width: 74.22222%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-9-8 {
        width: 85.11111%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-10,
    .CSPvNext .margin-row-fluid>.bp0-col-10-1 {
        width: 7.8%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-10-3 {
        width: 27.4%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-10-4 {
        width: 37.2%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-10-6 {
        width: 56.8%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-10-7 {
        width: 66.6%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-10-8 {
        width: 76.4%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-10-9 {
        width: 86.2%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-11,
    .CSPvNext .margin-row-fluid>.bp0-col-11-1 {
        width: 6.90909%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-11-2 {
        width: 15.81818%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-11-3 {
        width: 24.72727%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-11-4 {
        width: 33.63636%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-11-5 {
        width: 42.54545%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-11-6 {
        width: 51.45455%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-11-7 {
        width: 60.36364%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-11-8 {
        width: 69.27273%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-11-9 {
        width: 78.18182%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-11-10 {
        width: 87.09091%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-12,
    .CSPvNext .margin-row-fluid>.bp0-col-12-1 {
        width: 6.16667%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-12-5 {
        width: 38.83333%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-12-7 {
        width: 55.16667%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-12-9 {
        width: 71.5%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-12-10 {
        width: 79.66667%
    }
    .CSPvNext .margin-row-fluid>.bp0-col-12-11 {
        width: 87.83333%
    }
}
@media screen and (min-width: 540px) and (max-width: 679px) {
    .CSPvNext .margin-row-fluid .span {
        width: 96%
    }
    .CSPvNext .margin-row-fluid .bp1-clear {
        clear: both
    }
    .CSPvNext .margin-row-fluid [class*="bp1-col"] {
        display: block
    }
    .CSPvNext .margin-row-fluid>.bp1-clear,
    .CSPvNext .margin-row-fluid>.bp1-col-1-1 {
        margin-left: 2%
    }
    .CSPvNext .margin-row-fluid .bp1-col-1-0,
    .CSPvNext .margin-row-fluid .bp1-col-2-0,
    .CSPvNext .margin-row-fluid .bp1-col-3-0,
    .CSPvNext .margin-row-fluid .bp1-col-4-0,
    .CSPvNext .margin-row-fluid .bp1-col-5-0,
    .CSPvNext .margin-row-fluid .bp1-col-6-0,
    .CSPvNext .margin-row-fluid .bp1-col-7-0,
    .CSPvNext .margin-row-fluid .bp1-col-8-0,
    .CSPvNext .margin-row-fluid .bp1-col-9-0,
    .CSPvNext .margin-row-fluid .bp1-col-10-0,
    .CSPvNext .margin-row-fluid .bp1-col-11-0,
    .CSPvNext .margin-row-fluid .bp1-col-12-0 {
        display: none
    }
    .CSPvNext .margin-row-fluid>.bp1-col-1,
    .CSPvNext .margin-row-fluid>.bp1-col-1-1,
    .CSPvNext .margin-row-fluid>.bp1-col-2-2,
    .CSPvNext .margin-row-fluid>.bp1-col-3-3,
    .CSPvNext .margin-row-fluid>.bp1-col-4-4,
    .CSPvNext .margin-row-fluid>.bp1-col-5-5,
    .CSPvNext .margin-row-fluid>.bp1-col-6-6,
    .CSPvNext .margin-row-fluid>.bp1-col-7-7,
    .CSPvNext .margin-row-fluid>.bp1-col-8-8,
    .CSPvNext .margin-row-fluid>.bp1-col-9-9,
    .CSPvNext .margin-row-fluid>.bp1-col-10-10,
    .CSPvNext .margin-row-fluid>.bp1-col-11-11,
    .CSPvNext .margin-row-fluid>.bp1-col-12-12 {
        width: 96%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-2,
    .CSPvNext .margin-row-fluid>.bp1-col-2-1,
    .CSPvNext .margin-row-fluid>.bp1-col-4-2,
    .CSPvNext .margin-row-fluid>.bp1-col-6-3,
    .CSPvNext .margin-row-fluid>.bp1-col-8-4,
    .CSPvNext .margin-row-fluid>.bp1-col-10-5,
    .CSPvNext .margin-row-fluid>.bp1-col-12-6 {
        width: 47%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-3,
    .CSPvNext .margin-row-fluid>.bp1-col-3-1,
    .CSPvNext .margin-row-fluid>.bp1-col-6-2,
    .CSPvNext .margin-row-fluid>.bp1-col-9-3,
    .CSPvNext .margin-row-fluid>.bp1-col-12-4 {
        width: 30.66667%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-3-2,
    .CSPvNext .margin-row-fluid>.bp1-col-6-4,
    .CSPvNext .margin-row-fluid>.bp1-col-9-6,
    .CSPvNext .margin-row-fluid>.bp1-col-12-8 {
        width: 63.33333%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-4,
    .CSPvNext .margin-row-fluid>.bp1-col-4-1,
    .CSPvNext .margin-row-fluid>.bp1-col-8-2,
    .CSPvNext .margin-row-fluid>.bp1-col-12-3 {
        width: 22.5%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-4-3,
    .CSPvNext .margin-row-fluid>.bp1-col-8-6 {
        width: 71.5%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-5,
    .CSPvNext .margin-row-fluid>.bp1-col-5-1,
    .CSPvNext .margin-row-fluid>.bp1-col-10-2 {
        width: 17.6%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-5-2 {
        width: 37.2%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-5-3 {
        width: 56.8%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-5-4 {
        width: 76.4%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-6,
    .CSPvNext .margin-row-fluid>.bp1-col-6-1,
    .CSPvNext .margin-row-fluid>.bp1-col-12-2 {
        width: 14.33333%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-6-5 {
        width: 79.66667%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-7,
    .CSPvNext .margin-row-fluid>.bp1-col-7-1 {
        width: 12%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-7-2 {
        width: 26%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-7-3 {
        width: 40%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-7-4 {
        width: 54%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-7-5 {
        width: 68%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-7-6 {
        width: 82%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-8,
    .CSPvNext .margin-row-fluid>.bp1-col-8-1 {
        width: 10.25%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-8-3 {
        width: 34.75%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-8-5 {
        width: 59.25%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-8-7 {
        width: 83.75%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-9,
    .CSPvNext .margin-row-fluid>.bp1-col-9-1 {
        width: 8.88889%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-9-2 {
        width: 19.77778%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-9-4 {
        width: 41.55556%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-9-5 {
        width: 52.44444%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-9-7 {
        width: 74.22222%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-9-8 {
        width: 85.11111%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-10,
    .CSPvNext .margin-row-fluid>.bp1-col-10-1 {
        width: 7.8%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-10-3 {
        width: 27.4%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-10-4 {
        width: 37.2%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-10-6 {
        width: 56.8%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-10-7 {
        width: 66.6%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-10-8 {
        width: 76.4%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-10-9 {
        width: 86.2%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-11,
    .CSPvNext .margin-row-fluid>.bp1-col-11-1 {
        width: 6.90909%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-11-2 {
        width: 15.81818%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-11-3 {
        width: 24.72727%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-11-4 {
        width: 33.63636%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-11-5 {
        width: 42.54545%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-11-6 {
        width: 51.45455%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-11-7 {
        width: 60.36364%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-11-8 {
        width: 69.27273%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-11-9 {
        width: 78.18182%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-11-10 {
        width: 87.09091%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-12,
    .CSPvNext .margin-row-fluid>.bp1-col-12-1 {
        width: 6.16667%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-12-5 {
        width: 38.83333%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-12-7 {
        width: 55.16667%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-12-9 {
        width: 71.5%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-12-10 {
        width: 79.66667%
    }
    .CSPvNext .margin-row-fluid>.bp1-col-12-11 {
        width: 87.83333%
    }
}
@media screen and (min-width: 680px) and (max-width: 899px) {
    .CSPvNext .margin-row-fluid .span {
        width: 96%
    }
    .CSPvNext .margin-row-fluid>.span {
        width: 96%
    }
    .CSPvNext .margin-row-fluid .bp2-clear {
        clear: both
    }
    .CSPvNext .margin-row-fluid [class*="bp2-col"] {
        display: block
    }
    .CSPvNext .margin-row-fluid>.bp2-clear,
    .CSPvNext .margin-row-fluid>.bp2-col-1-1 {
        margin-left: 2%
    }
    .CSPvNext .margin-row-fluid .bp2-col-1-0,
    .CSPvNext .margin-row-fluid .bp2-col-2-0,
    .CSPvNext .margin-row-fluid .bp2-col-3-0,
    .CSPvNext .margin-row-fluid .bp2-col-4-0,
    .CSPvNext .margin-row-fluid .bp2-col-5-0,
    .CSPvNext .margin-row-fluid .bp2-col-6-0,
    .CSPvNext .margin-row-fluid .bp2-col-7-0,
    .CSPvNext .margin-row-fluid .bp2-col-8-0,
    .CSPvNext .margin-row-fluid .bp2-col-9-0,
    .CSPvNext .margin-row-fluid .bp2-col-10-0,
    .CSPvNext .margin-row-fluid .bp2-col-11-0,
    .CSPvNext .margin-row-fluid .bp2-col-12-0 {
        display: none
    }
    .CSPvNext .margin-row-fluid>.bp2-col-1,
    .CSPvNext .margin-row-fluid>.bp2-col-1-1,
    .CSPvNext .margin-row-fluid>.bp2-col-2-2,
    .CSPvNext .margin-row-fluid>.bp2-col-3-3,
    .CSPvNext .margin-row-fluid>.bp2-col-4-4,
    .CSPvNext .margin-row-fluid>.bp2-col-5-5,
    .CSPvNext .margin-row-fluid>.bp2-col-6-6,
    .CSPvNext .margin-row-fluid>.bp2-col-7-7,
    .CSPvNext .margin-row-fluid>.bp2-col-8-8,
    .CSPvNext .margin-row-fluid>.bp2-col-9-9,
    .CSPvNext .margin-row-fluid>.bp2-col-10-10,
    .CSPvNext .margin-row-fluid>.bp2-col-11-11,
    .CSPvNext .margin-row-fluid>.bp2-col-12-12 {
        width: 96%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-2,
    .CSPvNext .margin-row-fluid>.bp2-col-2-1,
    .CSPvNext .margin-row-fluid>.bp2-col-4-2,
    .CSPvNext .margin-row-fluid>.bp2-col-6-3,
    .CSPvNext .margin-row-fluid>.bp2-col-8-4,
    .CSPvNext .margin-row-fluid>.bp2-col-10-5,
    .CSPvNext .margin-row-fluid>.bp2-col-12-6 {
        width: 47%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-3,
    .CSPvNext .margin-row-fluid>.bp2-col-3-1,
    .CSPvNext .margin-row-fluid>.bp2-col-6-2,
    .CSPvNext .margin-row-fluid>.bp2-col-9-3,
    .CSPvNext .margin-row-fluid>.bp2-col-12-4 {
        width: 30.66667%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-3-2,
    .CSPvNext .margin-row-fluid>.bp2-col-6-4,
    .CSPvNext .margin-row-fluid>.bp2-col-9-6,
    .CSPvNext .margin-row-fluid>.bp2-col-12-8 {
        width: 63.33333%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-4,
    .CSPvNext .margin-row-fluid>.bp2-col-4-1,
    .CSPvNext .margin-row-fluid>.bp2-col-8-2,
    .CSPvNext .margin-row-fluid>.bp2-col-12-3 {
        width: 22.5%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-4-3,
    .CSPvNext .margin-row-fluid>.bp2-col-8-6 {
        width: 71.5%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-5,
    .CSPvNext .margin-row-fluid>.bp2-col-5-1,
    .CSPvNext .margin-row-fluid>.bp2-col-10-2 {
        width: 17.6%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-5-2 {
        width: 37.2%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-5-3 {
        width: 56.8%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-5-4 {
        width: 76.4%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-6,
    .CSPvNext .margin-row-fluid>.bp2-col-6-1,
    .CSPvNext .margin-row-fluid>.bp2-col-12-2 {
        width: 14.33333%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-6-5 {
        width: 79.66667%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-7,
    .CSPvNext .margin-row-fluid>.bp2-col-7-1 {
        width: 12%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-7-2 {
        width: 26%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-7-3 {
        width: 40%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-7-4 {
        width: 54%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-7-5 {
        width: 68%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-7-6 {
        width: 82%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-8,
    .CSPvNext .margin-row-fluid>.bp2-col-8-1 {
        width: 10.25%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-8-3 {
        width: 34.75%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-8-5 {
        width: 59.25%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-8-7 {
        width: 83.75%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-9,
    .CSPvNext .margin-row-fluid>.bp2-col-9-1 {
        width: 8.88889%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-9-2 {
        width: 19.77778%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-9-4 {
        width: 41.55556%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-9-5 {
        width: 52.44444%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-9-7 {
        width: 74.22222%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-9-8 {
        width: 85.11111%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-10,
    .CSPvNext .margin-row-fluid>.bp2-col-10-1 {
        width: 7.8%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-10-3 {
        width: 27.4%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-10-4 {
        width: 37.2%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-10-6 {
        width: 56.8%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-10-7 {
        width: 66.6%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-10-8 {
        width: 76.4%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-10-9 {
        width: 86.2%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-11,
    .CSPvNext .margin-row-fluid>.bp2-col-11-1 {
        width: 6.90909%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-11-2 {
        width: 15.81818%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-11-3 {
        width: 24.72727%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-11-4 {
        width: 33.63636%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-11-5 {
        width: 42.54545%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-11-6 {
        width: 51.45455%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-11-7 {
        width: 60.36364%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-11-8 {
        width: 69.27273%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-11-9 {
        width: 78.18182%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-11-10 {
        width: 87.09091%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-12,
    .CSPvNext .margin-row-fluid>.bp2-col-12-1 {
        width: 6.16667%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-12-5 {
        width: 38.83333%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-12-7 {
        width: 55.16667%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-12-9 {
        width: 71.5%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-12-10 {
        width: 79.66667%
    }
    .CSPvNext .margin-row-fluid>.bp2-col-12-11 {
        width: 87.83333%
    }
}
html {
    direction: ltr
}
body {
    font-family: wf_segoe-ui_normal, 'Segoe UI', Segoe, 'Segoe WP', Tahoma, Verdana, Arial, sans-serif;
    width: 100%;
    margin: 0;
    padding: 0
}
.CMSvNextComp,
.CMSvNextComp div,
.CMSvNextComp h1,
.CMSvNextComp h2,
.CMSvNextComp h3,
.CMSvNextComp h4,
.CMSvNextComp h5,
.CMSvNextComp h6,
.CMSvNextComp li,
.CMSvNextComp ol,
.CMSvNextComp p,
.CMSvNextComp ul {
    margin: 0;
    padding: 0
}
.CMSvNextComp a {
    cursor: pointer
}
.CMSvNextComp a:link,
.CMSvNextComp a:visited {
    text-decoration: none
}
.CMSvNextComp a:hover,
.CMSvNextComp a:active {
    text-decoration: underline
}
.CMSvNextComp h1,
.CMSvNextComp h2,
.CMSvNextComp h3,
.CMSvNextComp h4,
.CMSvNextComp h5,
.CMSvNextComp h6 {
    font-weight: normal;
    font-size: 100%
}
.CMSvNextComp img {
    border: none
}
.CMSvNextComp ol,
.CMSvNextComp ul {
    list-style-type: none
}
.CMSvNextComp .cssClear {
    clear: both;
    height: 0;
    margin: 0;
    padding: 0;
    _line-height: 0
}
.CMSvNextComp .png-img {
    position: relative;
    overflow: hidden
}
.CMSvNextComp .png-img span {
    position: absolute;
    top: -9999px
}
.CMSvNextComp .png-img div {
    background-position: 0 center;
    background-repeat: no-repeat
}
.CMSvNextComp .png-img .bg-img {
    display: block;
    _display: none
}
.CMSvNextComp .png-img .filter-img {
    display: none;
    _display: block
}
.CMSvNextComp .mscom-left {
    float: left
}
.CMSvNextComp .mscom-right {
    float: right
}
.CMSvNextComp .mscom-textalign-left {
    text-align: left
}
.CMSvNextComp .mscom-textalign-right {
    text-align: right
}
.CMSvNextComp .mscom-fullwidth {
    width: 100% !important
}
.CMSvNextComp .mscom-remove-margin-left {
    margin-left: 0 !important
}
.CMSvNextComp .mscom-hide {
    display: none !important
}
.CMSvNextComp .mscom-show {
    display: block !important
}
.CMSvNextComp .mscom-maxwidth {
    margin: 0 auto;
    max-width: 1600px
}
.CMSvNextComp .mscom-container-maxwidth {
    margin: 0 auto;
    max-width: 1180px
}
.CMSvNextComp img,
.CMSvNextComp video {
    height: auto;
    max-width: 100%
}
@media screen and (max-width: 539px) {
    .CMSvNextComp .mscom-fullwidth-view-1 {
        width: 100% !important;
        margin: 0 !important
    }
}
.icon-left,
.icon-right,
.icon-menu,
.icon-search,
.icon-plus,
.icon-minus,
.icon-circledown,
.icon-globe,
.icon-up {
    font-family: wf_icons_normal
}
.fontface .icon-left:after {
    content: '←'
}
.fontface .icon-right:after {
    content: '→'
}
.fontface .icon-menu:after {
    content: '≡'
}
.fontface .icon-search:after {
    content: '⌕'
}
.fontface .icon-plus:after {
    content: '+'
}
.fontface .icon-minus:after {
    content: '-'
}
.fontface .icon-circledown:after {
    content: '∨'
}
.fontface .icon-globe:after {
    content: '●'
}
.fontface .icon-close:after {
    content: '×'
}
.fontface .icon-up:after {
    content: '↑'
}
.nofontface .icon-left,
.nofontface .icon-right,
.nofontface .icon-menu,
.nofontface .icon-search,
.nofontface .icon-up,
.nofontface .icon-globe {
    background-position: center center;
    background-repeat: no-repeat;
    display: inline-block
}
.lt-ie8 .icon-left,
.mscom-nonjs .icon-left,
.nofontface .icon-left {
    background-image: url("//c.s-microsoft.com/en-us/CMSImages/left-arrow-black.png?version=4C7F88BB-D160-A52D-9421-EEA8CA80B11B");
    height: 25px;
    width: 25px
}
.lt-ie8 .icon-right,
.mscom-nonjs .icon-right,
.nofontface .icon-right {
    background-image: url("//c.s-microsoft.com/en-us/CMSImages/left-arrow-black.png?version=4C7F88BB-D160-A52D-9421-EEA8CA80B11B");
    height: 25px;
    width: 25px
}
.lt-ie8 .icon-left,
.mscom-nonjs .light-foreground .icon-left,
.nofontface .light-foreground .icon-left {
    background-image: url("//c.s-microsoft.com/en-us/CMSImages/left-arrow-white.png?version=0044E65E-E2B3-A2CC-644F-5F6463A034C7");
    height: 25px;
    width: 25px
}
.lt-ie8 .light-foreground .icon-right,
.mscom-nonjs .light-foreground .icon-right,
.nofontface .light-foreground .icon-right {
    background-image: url("//c.s-microsoft.com/en-us/CMSImages/left-arrow-white.png?version=0044E65E-E2B3-A2CC-644F-5F6463A034C7");
    height: 25px;
    width: 25px
}
.light-foreground .icon-left,
.light-foreground .icon-right {
    color: #fff
}
.lt-ie8 .icon-menu,
.mscom-nonjs .icon-menu,
.nofontface .icon-menu {
    background-image: url("//c.s-microsoft.com/en-us/CMSImages/list-blue.png?version=E0DC3EDD-4755-4B13-320F-0EB005E772D3");
    display: inline-block;
    height: 16px;
    width: 16px
}
.lt-ie8 .icon-search,
.mscom-nonjs .icon-search,
.nofontface .icon-search {
    background-image: url("//c.s-microsoft.com/en-us/CMSImages/search-button-blue.png?version=158C949C-D3DF-0BB9-E8F6-25304016AA41");
    display: inline-block;
    height: 16px;
    width: 16px
}
.lt-ie8 .icon-circledown,
.mscom-nonjs .icon-circledown,
.nofontface .icon-circledown {
    background-image: url("//c.s-microsoft.com/en-us/CMSImages/dropdown-arrow-white.png?version=86BBA961-D367-DE08-E486-F924392E0146");
    height: 6px;
    width: 8px
}
.lt-ie8 .icon-globe,
.mscom-nonjs .icon-globe,
.nofontface .icon-globe {
    background-image: url("//c.s-microsoft.com/en-us/CMSImages/globe-black.png?version=C1CB0753-5FBF-150A-58CB-65740B918D52");
    height: 24px;
    width: 24px
}
.lt-ie8 .icon-up,
.mscom-nonjs .icon-up,
.nofontface .icon-up {
    background-image: url("//c.s-microsoft.com/en-us/CMSImages/MSCOM_UpArrow_22x22_505050.png?version=D9A0C4B9-FDA7-0BA5-10B8-9252C49F6C9C");
    height: 22px;
    width: 22px
}
.mscom-styles-bold {
    font-weight: bold
}
.mscom-styles-italic {
    font-style: italic
}
.mscom-alert .mscom-alertitem {
    background: none repeat scroll 0 0 #1570a6;
    margin: 0 auto
}
.mscom-alert .mscom-alertitem,
.mscom-alert .mscom-alert-description a {
    color: #fff
}
.mscom-alert .mscom-alert-description {
    float: left
}
.mscom-alert .mscom-alert-link {
    float: right
}
.mscom-alert .center .mscom-alert-description {
    float: none;
    text-align: center
}
.mscom-alert .mscom-alert-red {
    background: none repeat scroll 0 0 #ba141a
}
.mscom-alert .mscom-alertitem:before,
.mscom-alert .mscom-alertitem:after {
    content: "";
    display: table;
    line-height: 0;
    clear: both;
    float: none
}
.mscom-alert .mscom-alert-text {
    clear: none;
    color: #fff;
    margin-left: 50px;
    margin-right: 15px
}
.mscom-alert .mscom-alert-close {
    vertical-align: middle;
    *padding-top: 5px
}
.lt-ie9 .mscom-alert {
    min-width: 1180px
}
.lt-ie9 .mscom-alert .mscom-alert-text {
    float: left
}
.lt-ie9 .mscom-alert .mscom-alert-close {
    float: left;
    width: 16px
}
a.mscom-siteIdentity:link,
a.mscom-siteIdentity:visited,
a.mscom-siteIdentity:active,
a.mscom-siteIdentity:hover {
    color: #333;
    font-size: 30px;
    text-decoration: none
}
@media screen and (max-width: 539px) {
    a.mscom-siteIdentity:link,
    a.mscom-siteIdentity:visited,
    a.mscom-siteIdentity:active,
    a.mscom-siteIdentity:hover {
        font-size: 20px
    }
}
.mscom-siteLogo {
    display: inline-block
}
.mscom-siteLogo .mscom-image {
    max-width: none
}
.lt-ie9 .mscom-siteLogo {
    display: block
}
.mscom-siteLogo-large .mscom-image {
    width: 115px;
    height: auto;
    margin-top: .5em
}
.lt-ie9 .mscom-siteLogo-large .mscom-image {
    width: 160px;
    margin-top: 0
}
@media screen and (min-width: 540px) {
    .mscom-siteLogo-large .mscom-image {
        width: 160px;
        margin-top: 0;
        margin-top: .5em
    }
}
.mscom-siteLogo-small .mscom-image {
    width: 94px;
    height: auto
}
@media screen and (max-width: 539px) {
    .mscom-siteIdentity {
        font-size: 20px
    }
    .mscom-siteLogo-small .mscom-image {
        margin-top: 1em
    }
}
.mscom-search-image {
    height: 80px;
    width: 20px
}
.mscom-search-DropdownContainer,
.mscom-search-NonJsDropdownContainer,
.mscom-search-Sources,
.mscom-search-Dropdown,
span.mscom-search-Border,
input.mscom-search-TextBox,
.mscom-search-Suggestions,
.mscom-search-history,
.mscom-search-quick-search {
    width: 100%
}
.mscom-search span.mscom-search-Border {
    position: relative;
    z-index: 998;
    background-color: #fff;
    border: solid 1px #969696;
    display: inline-block;
    _height: 22px;
    color: #000
}
.mscom-search input.mscom-search-TextBox {
    font-size: .75em;
    background-color: transparent;
    height: 21px;
    color: #000;
    margin-top: 1px;
    border: 0;
    padding-top: 2px;
    padding-bottom: 0;
    padding-left: 7px;
    padding-right: 3px;
    vertical-align: top;
    outline: 0;
    margin-left: 3px
}
.mscom-search input.mscom-search-Button {
    margin: 3px 1px 1px 1px;
    border: 0;
    cursor: pointer;
    overflow: hidden;
    vertical-align: top;
    border: 0;
    background-color: #fff;
    right: 2px;
    position: absolute;
    top: 0;
    margin-left: 5px;
    font-size: 0
}
.mscom-nonjs .mscom-search input.mscom-search-Button {
    right: 16px
}
.mscom-search .mscom-search-DropdownContainer {
    position: absolute;
    bottom: 0;
    *z-index: 85;
    left: 0;
    _position: static
}
.mscom-search .mscom-search-DropdownArrow {
    position: absolute;
    overflow: hidden;
    margin: 3px;
    cursor: pointer;
    display: inline-block;
    border: 0;
    margin-left: 0;
    max-width: none;
    top: -60px
}
.mscom-search .mscom-search-DropdownSection {
    padding: 0;
    margin: 0;
    display: block;
    height: 20px;
    overflow: hidden;
    position: relative;
    width: 16px
}
.mscom-search .mscom-search-Dropdown {
    font-size: 12.5px;
    display: none;
    position: absolute;
    top: 0;
    bottom: 0;
    *z-index: 85;
    left: 0;
    _top: auto
}
@media screen and (-webkit-min-device-pixel-ratio: 0) {
    .mscom-search .mscom-search-dropdown-pin {
        top: 0
    }
}
.mscom-search .mscom-search-Sources {
    _position: absolute
}
.mscom-search .mscom-search-Sources,
.mscom-search .mscom-search-Suggestions,
.mscom-search .mscom-search-history,
.mscom-search .mscom-search-quick-search {
    position: relative;
    z-index: 85;
    background: #fff;
    left: -1px;
    _left: 0
}
.mscom-search .mscom-search-Sources,
.mscom-search .mscom-search-Suggestions,
.mscom-search .mscom-search-history {
    border: solid 1px #999
}
.mscom-search .mscom-search-Sources ul {
    _display: inline-block;
    *display: inline-block
}
.mscom-search .mscom-search-Sources li {
    padding-bottom: 1px;
    width: 100%
}
.mscom-search .mscom-search-Sources li a {
    display: block;
    position: relative;
    overflow: hidden;
    text-decoration: none;
    padding-left: 35px;
    padding-top: 5px;
    padding-bottom: 6px;
    text-align: left
}
.mscom-search .mscom-search-Sources li a.selected {
    background-color: #ededed
}
.mscom-search .mscom-search-Sources li a:hover,
.mscom-search .mscom-search-Sources li a:focus {
    _text-decoration: none
}
.mscom-search .mscom-search-Sources li.currentScope .mscom-search-Check {
    top: -34px
}
.mscom-search .mscom-search-Sources li .mscom-search-Check {
    display: inline-block;
    position: absolute;
    left: 10px;
    vertical-align: top;
    margin-right: 4px;
    top: 30px;
    max-width: none
}
.mscom-search .mscom-search-Sources li .mscom-search-Text {
    display: table-cell;
    white-space: normal;
    *text-align: justify
}
.mscom-search .mscom-search-Suggestions,
.mscom-search .mscom-search-history,
.mscom-search .mscom-search-quick-search {
    display: none;
    border-bottom: none;
    overflow: hidden;
    border-bottom-style: none;
    *text-align: justify !important
}
.mscom-search .mscom-search-Suggestions li,
.mscom-search .mscom-search-history li,
.mscom-search .mscom-search-quick-search li {
    padding-bottom: 1px
}
.mscom-search .mscom-search-Suggestions li a,
.mscom-search .mscom-search-history li a {
    display: block;
    text-decoration: none;
    padding: 6px 5px 6px 5px;
    _overflow: hidden
}
.mscom-search .mscom-search-Suggestions a.selected,
.mscom-search .mscom-search-history a.selected {
    text-decoration: underline
}
.mscom-search .mscom-search-Suggestions li .mscom-search-Text,
.mscom-search .mscom-search-history li .mscom-search-Text {
    display: table-cell;
    _display: inline-block;
    _white-space: normal
}
.mscom-search .mscom-search-Sources li.currentScope a {
    font-weight: bold
}
.mscom-search .mscom-search-Sources ul,
.mscom-search .mscom-search-Suggestions ul,
.mscom-search .mscom-search-history ul {
    padding: 2px 0 1px 0
}
.mscom-search .mscom-search-Suggestions li a,
.mscom-search .mscom-search-history li a {
    padding-left: 10px
}
.mscom-search .mscom-search-Sources li a:link,
.mscom-search .mscom-search-Sources li a:visited,
.mscom-search .mscom-search-Suggestions li a:link,
.mscom-search .mscom-search-Suggestions li a:visited {
    color: #333
}
.mscom-search .mscom-search-history li a:link,
.mscom-search .mscom-search-history li a:visited {
    color: #639
}
.mscom-search .mscom-search-Suggestions li a:hover,
.mscom-search .mscom-search-Suggestions li a:focus,
.mscom-search .mscom-search-Suggestions li a:active,
.mscom-search .mscom-search-Suggestions li a.selected,
.mscom-search .mscom-search-Sources li a:hover,
.mscom-search .mscom-search-Sources li a:focus,
.mscom-search .mscom-search-Sources li a:active,
.mscom-search .mscom-search-Sources li a.selected,
.mscom-search .mscom-search-history li a:hover,
.mscom-search .mscom-search-history li a:focus,
.mscom-search .mscom-search-history li a:active,
.mscom-search .mscom-search-history li a.selected {
    background-color: #1570a6;
    color: #fff;
    font-size: 12.5px
}
.mscom-search .mscom-search-Sources li.currentScope a:hover .mscom-search-Check,
.mscom-search .mscom-search-Sources li.currentScope a:focus .mscom-search-Check {
    top: -14px;
    *border: 0 solid #00f
}
.mscom-search .mscom-search-NonJsDropdownContainer {
    right: 2px;
    position: absolute;
    top: 0;
    float: left
}
.mscom-search .mscom-search-Container {
    padding-right: 50px;
    float: none;
    position: relative;
    overflow: hidden
}
strong {
    font-weight: bold
}
.mscom-search .mscom-search-quick-search {
    font-size: 1em;
    border-left: solid 1px #d2d2d2;
    border-right: solid 1px #d2d2d2;
    border-top: none;
    border-bottom: none
}
.mscom-search .mscom-search-quick-search .quick-search-item {
    display: table;
    *display: inline-block;
    *zoom: 1;
    width: 100%;
    cursor: pointer;
    border-bottom: solid 1px #d2d2d2;
    padding-top: 5px;
    padding-bottom: 6px;
    padding-left: 5px
}
.mscom-search .mscom-search-quick-search .quick-search-thumbnail img {
    width: 100%;
    height: auto
}
.mscom-search .mscom-search-quick-search .quick-search-thumbnail img.error {
    display: none
}
.mscom-search .mscom-search-quick-search .quick-search-item .quick-search-thumbnail {
    display: table-cell;
    *display: inline-block;
    *zoom: 1;
    *float: left;
    width: 45px;
    vertical-align: top;
    padding-top: 5px
}
.mscom-search .mscom-search-quick-search .quick-search-item .quick-search-caption {
    display: table-cell;
    *display: inline-block;
    *zoom: 1;
    vertical-align: top;
    padding-left: 10px;
    padding-right: 10px
}
.mscom-search .mscom-search-quick-search .quick-search-item .quick-search-caption p {
    font-size: .75em;
    line-height: 1.35em;
    padding-right: 5px
}
.mscom-search .mscom-search-quick-search .quick-search-links {
    padding-bottom: 5px;
    padding-top: 5px;
    font-size: .75em
}
.mscom-search .mscom-search-quick-search .quick-search-links a {
    color: #124191;
    line-height: 1.2em;
    font-weight: bold
}
.mscom-search .mscom-search-quick-search .quick-search-links li {
    line-height: 1.5em
}
.mscom-search .mscom-search-quick-search .quick-search-item h3 {
    display: block;
    text-transform: uppercase;
    margin-bottom: 5px;
    font-weight: bold;
    line-height: 1.23em;
    font-size: .825em
}
.mscom-search .mscom-search-quick-search .quick-search-item.selected,
.mscom-search .mscom-search-quick-search .quick-search-item:hover,
.mscom-search .mscom-search-quick-search .quick-search-item:focus,
.mscom-search .mscom-search-quick-search .quick-search-item:active {
    background-color: #1570a6;
    color: #fff;
    text-decoration: none
}
.mscom-search .mscom-search-quick-search .quick-search-item.selected a,
.mscom-search .mscom-search-quick-search .quick-search-item.selected h3,
.mscom-search .mscom-search-quick-search .quick-search-item.selected p,
.mscom-search .mscom-search-quick-search .quick-search-item:hover a,
.mscom-search .mscom-search-quick-search .quick-search-item:hover h3,
.mscom-search .mscom-search-quick-search .quick-search-item:hover p,
.mscom-search .mscom-search-quick-search .quick-search-item:focus a,
.mscom-search .mscom-search-quick-search .quick-search-item:focus h3,
.mscom-search .mscom-search-quick-search .quick-search-item:focus p,
.mscom-search .mscom-search-quick-search .quick-search-item:active a,
.mscom-search .mscom-search-quick-search .quick-search-item:active h3,
.mscom-search .mscom-search-quick-search .quick-search-item:active p {
    color: #fff
}
.mscom-accountcontrol-container {
    position: relative;
    display: inline-block
}
a.mscom-account-text {
    background-color: #fff;
    position: relative;
    z-index: 1000;
    border-color: #fff;
    border-image: none;
    border-style: solid solid none;
    border-width: 1px
}
.selected a.mscom-account-text {
    border-color: #e0e0e0
}
.mscom-account-text a,
.mscom-account-text a:link,
.mscom-account-text a:visited,
.mscom-account-text a:active,
.mscom-account-text a:hover,
a.mscom-account-text,
a.mscom-account-text:link,
a.mscom-account-text:visited,
a.mscom-account-text:active,
a.mscom-account-text:hover {
    color: #0072c6;
    font-size: 13px;
    padding: 0 5px;
    text-decoration: none
}
a.mscom-account-viewonelink,
a.mscom-account-link {
    display: block
}
a.mscom-account-viewonelink::after,
a.mscom-account-link::after {
    font-size: .55em;
    content: "?";
    margin-left: .35em;
    color: #000;
    padding-left: 2px
}
.mscom-accountPosition {
    position: relative;
    z-index: 999
}
div.mscom-account {
    border: 1px solid #e0e0e0;
    background-color: #fff;
    position: absolute;
    top: -1px;
    width: 150px;
    z-index: 998;
    display: none;
    padding: 10px;
    right: 0
}
.mscom-account-items {
    padding: 10px
}
.mscom-accountcontrol-container {
    display: none
}
.selected .mscom-account-items {
    display: block
}
.mscom-account-viewone-items a {
    background: #1570a6;
    color: #fff;
    display: block;
    border-bottom: 1px solid #fff;
    padding: .5em .8em;
    text-decoration: none
}
a.mscom-account-viewonelink {
    display: block
}
.mscom-accountcontrol-viewone {
    display: none
}
@media screen and (min-width: 540px) {
    .mscom-accountcontrol-container {
        display: block
    }
    a.mscom-account-viewonelink {
        display: none
    }
    .mscom-header-search-section {
        display: block !important
    }
}
.mscom-signinout {
    margin-left: 10px
}
.mscom-signinout #idPPScarab {
    color: #0072c6;
    font-size: 13px;
    display: block
}
.mscom-signinout .mstHdr_userprofiletileimage {
    margin-left: 12px;
    height: 22px;
    width: 22px
}
.mscom-signinout #idPPScarab:hover,
.mscom-signinout #idPPScarab:active {
    text-decoration: none
}
.mscom-header {
    border-bottom: .3em solid #eee
}
.mscom-header:after {
    clear: both;
    display: block
}
.mscom-header .mscom-header-row-1 {
    margin-top: 2px;
    margin-bottom: .3em
}
.mscom-header .mscom-header-row-2 {
    margin-bottom: .3em
}
.mscom-header .mscom-header-section-1-1 {
    min-height: 1px
}
.mscom-header .mscom-header-section-1-2-2 {
    margin-left: .5em
}
.mscom-header .mscom-header-section-2-2 {
    font-size: 1.5em
}
.mscom-header .mscom-header-section-2-2 .mscom-show-navtoggle-text .screen-reader-text {
    height: auto;
    width: auto;
    position: relative;
    float: left;
    margin: 5% .2em auto auto;
    font-size: 80%
}
.mscom-header .mscom-header-section-2-2 a.mscom-header-navtogglelink:hover,
.mscom-header .mscom-header-section-2-2 a.mscom-header-searchtogglelink:hover {
    text-decoration: none
}
.mscom-header .mscom-header-section-2-4 {
    margin-top: .5em;
    width: 100%
}
.mscom-header .mscom-header-section-2-4 .span {
    margin-left: 0
}
.mscom-header .mscom-header-searchtogglelink {
    margin-left: .625em;
    height: 1.25em;
    width: 1.25em;
    overflow: hidden;
    position: relative;
    display: inline-block
}
.mscom-header .mscom-header-searchtogglelink img {
    top: 0;
    position: relative
}
.mscom-header .mscom-header-navtogglelink img {
    height: 1.25em;
    width: 1.25em
}
.mscom-header .row-fluid .mscom-header-nav-section {
    *line-height: 2.5em
}
.mscom-header {
    margin-top: 1em
}
.mscom-header .mscom-header-section-2-4 {
    width: 80%
}
@media screen and (min-width: 540px) {
    .mscom-header-nav-section {
        display: block !important
    }
}
@media screen and (max-width: 539px) {
    .mscom-header .mscom-header-row-2 .mscom-header-section-2-3 {
        margin-left: 0
    }
    .mscom-header .mscom-header-search-section.span {
        margin-left: 0
    }
    .mscom-header .row-fluid .mscom-header-nav-section {
        display: none
    }
    .mscom-header .row-fluid .mscom-header-show-nav-section {
        display: block
    }
    .mscom-header .mscom-header-section-2-4 {
        width: 100%
    }
    .mscom-header {
        margin-top: 0
    }
}
.lt-ie9 .mscom-header {
    min-width: 1180px
}
.mscom-header .mscom-findstorelink {
    color: #1570a6
}
.mscom-header .mscom-header-storelink-section {
    line-height: 2.5em
}
.lt-ie9 .mscom-header .mscom-header-storelink-section {
    line-height: 1em
}
.mscom-header .mscom-header-storelink-section-topbar {
    font-size: 14.4px
}
.mscom-header .mscom-findstorelink:hover {
    color: #083047;
    text-decoration: underline
}
@media screen and (max-width: 539px) {
    .mscom-header .mscom-header-row-0 {
        background-color: #eee
    }
    .CMSvNextComp .mscom-header {
        margin-top: 0
    }
    .mscom-header .mscom-header-section-1-1 {
        width: 0%
    }
    .mscom-header .mscom-header-section-1-2 {
        width: 96%
    }
    .mscom-header .mscom-header-storelink-section-topbar {
        padding-right: 2%
    }
}
.navIe7 {
    *position: relative;
    *z-index: 996
}
.mscom-nav .mscom-nav-container {
    margin: 0 0 0 -.93em
}
.mscom-nav .mscom-navitem {
    display: inline-block;
    *display: inline;
    *zoom: 1
}
.mscom-nav .selected {
    background: #eee
}
.mscom-nav .mscom-nav-item-link {
    text-decoration: none;
    padding: 0 .93em;
    color: #1a1a1a;
    font-size: 1em;
    white-space: nowrap
}
.mscom-nav .mscom-nav-item-link:hover {
    color: #0060a6;
    text-decoration: none
}
.mscom-nav .mscom-nav-item-flyout-link-icon:after {
    font-size: .688em;
    content: '∨';
    margin-left: .35em;
    font-family: "wf_icons_normal"
}
.mscom-nav .mscom-nav-flyout {
    background-color: #eee;
    position: absolute;
    width: 100%;
    *width: 101%;
    *left: -.93em;
    border-left: medium none;
    left: 0;
    opacity: 1;
    display: none;
    z-index: 996;
    *top: 43px
}
.lt-ie8 .top-level-link:focus {
    text-decoration: none
}
.mscom-nav .mscom-nav-flyout .mscom-nav-layout {
    padding: 1.35em 0;
    *padding: 1.35em .93em;
    margin: 0 auto;
    max-width: 1180px;
    width: 96%
}
.mscom-nav .mscom-nav-layout li {
    display: block;
    margin: 0 0 .7em;
    font-size: .9em
}
.mscom-nav .mscom-nav-layout li.heading {
    margin: 0 0 1.35em;
    font-size: 1em;
    font-weight: normal
}
.mscom-nav .mscom-nav-layout li a {
    color: #1570a6;
    text-decoration: none
}
.mscom-nav .mscom-nav-layout li a:hover {
    color: #083047;
    text-decoration: underline;
    height: 100%
}
.mscom-nav .mscom-nav-flyout .mscom-nav-linklayout-horizontal ul li {
    display: inline-block;
    padding: 0 .93em
}
.mscom-nav .mscom-nav-flyout .mscom-nav-linklayout-horizontal ul li:first-child {
    padding-left: 0
}
@media screen and (min-width: 540px) {
    .mscom-nav .mscom-nav-item-link {
        line-height: 2.5em
    }
    .mscom-nav li.mscom-navitem-keyline {
        border-bottom: .3em solid #1570a6;
        margin-bottom: -4px
    }
    .mscom-nav .mscom-nav-layout ul {
        margin-bottom: 2.5em
    }
    .mscom-nav .mscom-nav-imagelayout li a {
        line-height: 0;
        font-size: 0;
        color: transparent
    }
}
@media screen and (max-width: 539px) {
    .mscom-nav .mscom-nav-container {
        margin: 0;
        background-color: #1570a6
    }
    .mscom-nav .mscom-nav-container a {
        color: #fff;
        font-size: 1em;
        text-decoration: none !important
    }
    .mscom-nav .selected {
        background: #083047
    }
    .mscom-nav .mscom-nav-flyout {
        border-left: .5em solid #083047;
        background-color: #1570a6;
        position: relative;
        width: auto
    }
    .mscom-nav .mscom-nav-flyout .mscom-nav-layout {
        padding: 0
    }
    .mscom-nav .mscom-nav-flyout .mscom-nav-layout div.span {
        width: 100% !important;
        margin: 0 !important
    }
    .mscom-nav .mscom-navitem,
    .mscom-nav .mscom-nav-item-link,
    .mscom-nav .mscom-nav-flyout .mscom-nav-layout li a,
    .mscom-nav .mscom-nav-flyout .mscom-nav-linklayout-horizontal ul li {
        display: block
    }
    .mscom-nav .mscom-nav-flyout .mscom-nav-layout li {
        margin: 0 0 0
    }
    .mscom-nav .mscom-nav-flyout .mscom-nav-layout li.heading {
        display: none
    }
    .mscom-nav .mscom-nav-layout li {
        font-size: 1em
    }
    .mscom-nav .mscom-nav-item-link:hover,
    .mscom-nav .mscom-nav-layout li a:hover {
        background-color: #083047;
        color: #fff
    }
    .mscom-nav .mscom-nav-item-link,
    .mscom-nav .mscom-nav-flyout .mscom-nav-layout li a {
        border-bottom: 1px solid #fff;
        color: #fff;
        display: block;
        padding: .5em .8em
    }
    .mscom-nav .mscom-nav-flyout .mscom-nav-linklayout-horizontal ul li {
        display: block;
        padding: 0
    }
    .mscom-nav .mscom-nav-flyout .mscom-nav-imagelayout a img {
        display: none
    }
}
.lt-ie8 .mscom-nav-item-flyout-link-icon,
.nofontface .mscom-nav-item-flyout-link-icon {
    background-image: url("CMSImages/Caret_Gray.png?version=07593BEE-0203-BA88-C4A8-AD16F380F02A");
    height: 5px;
    width: 8px;
    display: inline-block;
    margin-left: .35em;
    margin-bottom: 5px
}
.lt-ie8 .mscom-nav-item-link:hover .mscom-nav-item-flyout-link-icon,
.nofontface .mscom-nav-item-link:hover .mscom-nav-item-flyout-link-icon {
    background-image: url("CMSImages/Caret_Blue.png?version=32461B33-0296-96C1-2625-042C56BFCF9A")
}
.CMSvNextComp div.mscom-locale-flyout {
    display: none;
    margin: 0 auto;
    max-width: 1180px
}
.mscom-footer {
    background-color: #eee
}
.mscom-footer .mscom-footer-row-1 {
    padding-top: 2.5%
}
.mscom-footer .mscom-footer-row-2 {
    padding-top: 1em
}
.mscom-footer .mscom-footer-row-3 {
    padding-bottom: .85%
}
.mscom-footer a {
    color: #1570a6;
    font-size: .9em
}
.mscom-footer a:focus,
.mscom-footer a:hover {
    color: #083047
}
.mscom-footer .heading {
    font-size: 1em;
    font-weight: normal;
    margin-bottom: .65em
}
.mscom-footer ul {
    margin-top: 1.5em
}
.mscom-footer li {
    line-height: 1.25;
    display: block;
    margin: 0 0 .25em;
    font-size: 1em
}
.mscom-footer .mscom-footer-productlist li {
    margin-bottom: .75em
}
.mscom-footer .mscom-footer-productlist a {
    color: #1a1a1a;
    font-size: 1em
}
.mscom-footer .mscom-footer-productlist img {
    display: inline-block;
    height: 26px;
    width: 26px;
    margin-bottom: -.45em;
    margin-right: 10px
}
.mscom-footer .mscom-footer-localepicker {
    margin-bottom: 1.5em
}
.mscom-footer .mscom-footer-localepicker span.icon-globe {
    font-size: 1.5em;
    line-height: 1.5em;
    float: left;
    margin-right: 10px;
    text-decoration: none;
    position: relative;
    top: -.22em
}
.mscom-footer .mscom-footer-defaultlink li {
    display: inline-block;
    *display: inline;
    margin-right: 1.5em
}
.mscom-footer .mscom-footer-defaultlink li a {
    *display: inline
}
.mscom-footer .mscom-footer-defaultlink .heading {
    font-size: .813em
}
.mscom-footer .mscom-footer-productlist {
    margin-bottom: 1em
}
.mscom-footer .mscom-footer-mslogo img {
    width: 85px;
    height: 16px
}
.mscom-footer .mscom-footer-defaultlink ul:first-child {
    margin-top: 0
}
@media screen and (min-width: 540px) {
    .mscom-footer .mscom-footer-defaultlink li {
        margin-right: 2%
    }
    .mscom-footer ul:first-child {
        margin-top: 0
    }
}
@media screen and (max-width: 900px) {
    .mscom-footer .mscom-footer-row-2 {
        padding-bottom: .7em
    }
}
@media screen and (min-width: 900px) {
    .mscom-footer .mscom-footer-mslogo,
    .mscom-footer .mscom-footer-defaultlink {
        text-align: right
    }
    .mscom-footer .mscom-footer-localepicker {
        margin-bottom: 0
    }
    .mscom-footer .mscom-footer-productlist {
        margin-bottom: 0
    }
    .mscom-footer .mscom-footer-defaultlink li {
        margin-left: 2%;
        margin-right: 0
    }
    .mscom-footer .mscom-footer-defaultlink li:first-child {
        margin-left: 0
    }
}
.lt-ie9 .mscom-footer {
    min-width: 1180px
}
.lt-ie9 .mscom-footer .mscom-footer-mslogo,
.lt-ie9 .mscom-footer .mscom-footer-defaultlink {
    text-align: right
}
.lt-ie9 .mscom-footer .mscom-footer-defaultlink li {
    margin-left: 2%;
    margin-right: 0
}
.lt-ie9 .mscom-footer .mscom-footer-defaultlink li:first-child {
    margin-left: 0
}
.lt-ie8 .mscom-footer .mscom-footer-localepicker span.icon-globe {
    display: static
}
.features {
    clear: both
}
.features p {
    font-size: .9em;
    margin-bottom: 0
}
.features img {
    margin-right: 4%;
    margin-top: .25em;
    width: 41%;
    float: left
}
.features .span {
    margin-bottom: 3%
}
.features a {
    -webkit-transition: color .35s ease-in;
    -moz-transition: color .35s ease-in;
    -ms-transition: color .35s ease-in;
    -o-transition: color .35s ease-in;
    transition: none
}
.features a img {
    -webkit-transition: opacity .35s ease-in;
    -moz-transition: opacity .35s ease-in;
    -ms-transition: opacity .35s ease-in;
    -o-transition: opacity .35s ease-in;
    transition: opacity .35s ease-in
}
.features a:hover {
    -webkit-transition: color .15s ease-in;
    -moz-transition: color .15s ease-in;
    -ms-transition: color .15s ease-in;
    -o-transition: color .15s ease-in;
    transition: color .15s ease-in
}
.features a img:hover {
    text-decoration: none;
    -webkit-transition: opacity .15s ease-in;
    -moz-transition: opacity .15s ease-in;
    -ms-transition: opacity .15s ease-in;
    -o-transition: opacity .15s ease-in;
    transition: opacity .15s ease-in
}
.features a:hover img,
.features a img:hover img {
    opacity: .85
}
@media screen and (min-width: 680px) {
    .features img {
        width: 100%;
        margin: 0 0 .25em;
        float: none;
        padding-top: 0
    }
    .features p {
        margin-bottom: 1.5em
    }
}
.cssanimations .features {
    -webkit-animation: fadeIn;
    -moz-animation: fadeIn;
    -o-animation: fadeIn;
    animation: fadeIn;
    -webkit-animation-duration: 1.5s;
    -moz-animation-duration: 1.5s;
    -o-animation-duration: 1.5s;
    animation-duration: 1.5s
}
.cssanimations .mscom-pivot-flyout .list-of-links {
    animation: IE11FixfadeIn;
    animation-duration: 1.51s
}
.cssanimations .features .grid-unit {
    position: relative;
    -webkit-animation: slideIn;
    -moz-animation: slideIn;
    -o-animation: slideIn;
    animation: slideIn
}
.features .grid-unit:nth-of-type(1) {
    -webkit-animation-duration: .225s;
    -moz-animation-duration: .225s;
    -o-animation-duration: .225s;
    animation-duration: .225s
}
.features .grid-unit:nth-of-type(2) {
    -webkit-animation-duration: .45s;
    -moz-animation-duration: .45s;
    -o-animation-duration: .45s;
    animation-duration: .45s
}
.features .grid-unit:nth-of-type(3) {
    -webkit-animation-duration: .675s;
    -moz-animation-duration: .675s;
    -o-animation-duration: .675s;
    animation-duration: .675s
}
.features .grid-unit:nth-of-type(4) {
    -webkit-animation-duration: .9s;
    -moz-animation-duration: .9s;
    -o-animation-duration: .9s;
    animation-duration: .9s
}
.lt-ie9 .features img {
    width: 100%;
    margin: 0 0 .25em;
    float: none;
    padding-top: 0
}
.features.color-box li {
    margin-bottom: 3%;
    background-color: #1570a6
}
.features.color-box li a p {
    margin: .5em 1em
}
.features.color-box li img {
    margin-top: 0
}
.features.color-box li a {
    color: #fff
}
@media screen and (min-width: 680px) {
    .features.color-box li a p {
        width: 92%;
        margin: -4px auto .65em
    }
}
.slideshow-news {
    background-color: #0d698d;
    background-color: rgba(0, 0, 0, .16);
    display: block;
    float: left;
    width: 100%;
    -webkit-transition: background .2s ease-in;
    -moz-transition: background .2s ease-in;
    -ms-transition: background .2s ease-in;
    -o-transition: background .2s ease-in;
    transition: background .2s ease-in;
    *zoom: 1;
    margin-bottom: 1em
}
.slideshow-news:after {
    content: "";
    display: table;
    clear: both
}
.slideshow-news:hover {
    background-color: #0a5471;
    background-color: rgba(0, 0, 0, .26);
    text-decoration: none
}
.slideshow-news a {
    overflow: hidden
}
.slideshow-news .prev-next {
    display: none
}
.slideshow-news li {
    display: block
}
.slideshow-news a {
    -webkit-box-sizing: border-box;
    -moz-box-sizing: border-box;
    box-sizing: border-box;
    border: 1px solid transparent;
    display: block;
    padding: 1em;
    text-decoration: none
}
.slideshow-news img {
    float: left;
    margin: 0 1.5em 0 0;
    max-width: 130px
}
.slideshow-news a .pubdate {
    margin-right: .2em
}
.slideshow-news .title {
    font-size: 1.167em;
    display: block;
    line-height: 1.4em;
    margin: 0;
    font-family: wf_segoe-ui_light, 'Segoe UI Light', 'Segoe WP Light', wf_segoe-ui_normal, 'Segoe UI', Segoe, 'Segoe WP', Tahoma, Verdana, Arial, sans-serif;
    font-weight: 300
}
.mac .slideshow-news .title {
    -webkit-font-smoothing: subpixel-antialiased
}
.slideshow-news p {
    margin-bottom: 0;
    line-height: 1.6em;
    _color: #1a1a1a
}
@media screen and (min-width: 560px) {
    .slideshow-news .title {
        font-size: 1.5em
    }
    .slideshow-news a {
        padding: 1.75em
    }
    .slideshow-news .widget-title {
        display: none
    }
}
@media screen and (min-width: 600px) {
    .slideshow-news img {
        max-width: 150px
    }
}
@media screen and (min-width: 900px) {
    .slideshow-news {
        margin-bottom: 0
    }
}
.lt-ie9 .slideshow-news .title {
    font-size: 1.5em;
    color: #fff
}
.lt-ie9 .slideshow-news .widget-title {
    display: none
}
.lt-ie9 .slideshow-news img {
    max-width: 150px;
    width: 150px;
    height: 112px
}
.lt-ie8 .slideshow-news {
    background-color: #0d698d
}
.mscom-nonjs.lt-ie9 .slideshow-news {
    background-color: #fff !important
}
.mscom-nonjs.lt-ie9 .slideshow-news .title {
    color: #000
}
.iem7 .slideshow-news,
.iem9 .slideshow-news,
.lt-ie8 .slideshow-news {
    float: none
}
.mscom-pivot .mscom-pivot-item {
    display: inline-block
}
.mscom-pivot .mscom-pivot-item-link {
    text-decoration: none;
    padding: 0 .93em;
    color: #1a1a1a;
    font-size: 1em
}
.mscom-pivot .mscom-pivot-item-link:hover {
    color: #0060a6;
    text-decoration: none
}
.mscom-pivot .selected .mscom-pivot-flyout {
    display: block
}
.mscom-pivot-nav .mscom-pivot-container {
    margin: 0 0 0 -.93em
}
.mscom-pivot-nav .selected {
    background: #eee
}
.mscom-pivot-nav .mscom-pivot-item-flyout-link::after {
    font-size: .55em;
    content: "?";
    margin-left: .35em
}
.mscom-pivot-nav .mscom-pivot-flyout {
    background-color: #eee;
    position: absolute;
    width: 100%;
    border-left: medium none;
    left: 0;
    opacity: 1;
    display: none;
    z-index: 996
}
.mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-layout {
    padding: 1.35em 0;
    margin: 0 auto;
    max-width: 1180px
}
.mscom-pivot-nav .mscom-nav-layout li {
    display: block;
    margin: 0 0 .7em;
    font-size: .9em
}
.mscom-pivot-nav .mscom-nav-layout li.heading {
    margin: 0 0 1.35em;
    font-size: 1em;
    font-weight: normal
}
.mscom-pivot-nav .mscom-nav-layout li a {
    color: #1570a6;
    text-decoration: none
}
.mscom-pivot-nav .mscom-nav-layout li a:hover {
    color: #083047;
    text-decoration: underline;
    height: 100%
}
.mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-linklayout-horizontal ul li {
    display: inline-block;
    padding: 0 .93em
}
.mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-linklayout-horizontal ul li:first-child {
    padding-left: 0
}
@media screen and (min-width: 540px) {
    .mscom-pivot-nav .mscom-pivot-item-link {
        line-height: 2.5em
    }
    .mscom-pivot-nav .mscom-nav-layout ul {
        margin-bottom: 2.5em
    }
    .mscom-pivot-nav .mscom-nav-imagelayout li a {
        line-height: 0;
        font-size: 0;
        color: transparent
    }
}
@media screen and (max-width: 539px) {
    .mscom-pivot-nav .mscom-pivot-container {
        margin: 0;
        background-color: #1570a6
    }
    .mscom-pivot-nav .mscom-pivot-container a {
        color: #fff;
        font-size: 1em;
        text-decoration: none !important
    }
    .mscom-pivot-nav .selected {
        background: #083047
    }
    .mscom-pivot-nav .mscom-pivot-flyout {
        border-left: .5em solid #083047;
        background-color: #1570a6;
        position: relative;
        width: auto
    }
    .mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-layout {
        padding: 0
    }
    .mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-layout div.span {
        width: 100% !important;
        margin: 0 !important
    }
    .mscom-pivot-nav .mscom-pivot-item,
    .mscom-pivot-nav .mscom-pivot-item-link,
    .mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-layout li a,
    .mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-linklayout-horizontal ul li {
        display: block
    }
    .mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-layout li {
        margin: 0 0 0
    }
    .mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-layout li.heading {
        display: none
    }
    .mscom-pivot-nav .mscom-pivot-layout li {
        font-size: 1em
    }
    .mscom-pivot-nav .mscom-pivot-item-link:hover,
    .mscom-pivot-nav .mscom-nav-layout li a:hover {
        background-color: #083047;
        color: #fff
    }
    .mscom-pivot-nav .mscom-pivot-item-link,
    .mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-layout li a {
        border-bottom: 1px solid #fff;
        color: #fff;
        display: block;
        padding: .5em .8em
    }
    .mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-linklayout-horizontal ul li {
        display: block;
        padding: 0
    }
    .mscom-pivot-nav .mscom-pivot-flyout .mscom-nav-imagelayout a img {
        display: none
    }
}
.mscom-pivot-tab .mscom-pivot-flyout .mscom-pivot-tab-layout a:hover .mscom-pivot-text,
.mscom-pivot-tab .mscom-pivot-flyout a:active .mscom-pivot-text {
    text-decoration: underline
}
.mscom-pivot-tab .mscom-pivot-flyout .mscom-pivot-tab-layout .mscom-pivot-text {
    color: #1570a6;
    font-size: .9em
}
.mscom-pivot-tab .mscom-pivot-flyout {
    border-left: medium none;
    left: 0;
    opacity: 1;
    width: 100%;
    display: none
}
.mscom-pivot-tab .mscom-pivot-item-flyout-link {
    border: 1px solid transparent;
    display: inline-block;
    font-size: 1em;
    margin: 0 0 .7em 0;
    padding: .1em 1%;
    position: relative;
    text-decoration: none
}
.mscom-pivot-tab a.selected,
.mscom-pivot-tab .selected .mscom-pivot-item-flyout-link {
    background-color: #1570a6;
    color: #fff
}
.mscom-pivot-tab a.selected:hover,
.mscom-pivot-tab .selected .mscom-pivot-item-flyout-link:hover {
    background-color: #083047;
    color: #fff;
    text-decoration: none
}
.mscom-pivot-tab .mscom-pivot-item-flyout-link:hover,
.mscom-pivot-tab .mscom-pivot-item-flyout-link:active {
    text-decoration: none
}
@media screen and (max-width: 680px) {
    .mscom-pivot-tab .mscom-pivot-flyout .mscom-pivot-tab-layout .mscom-pivot-image img {
        width: 95%
    }
    .mscom-pivot-tab .mscom-pivot-flyout .mscom-pivot-tab-layout .row-fluid .span {
        margin-bottom: .2em
    }
}
.grid-container {
    width: 96%;
    margin-left: auto;
    margin-right: auto;
    max-width: 1180px;
    *zoom: 1;
    background-repeat: no-repeat;
    background-position: center center
}
.row-padded {
    padding-top: 1.5em;
    padding-bottom: 1.5em
}
.row-padded-top {
    padding-top: 1.5em
}
.row-padded-bottom {
    padding-bottom: 1.5em
}
.border-top {
    border-top: 1px solid #eee
}
.border-bottom {
    border-bottom: 1px solid #eee
}
.row-cool-gray-3 {
    background: #d2d2d2
}
.row-grey {
    background: #eee
}
.row-blue {
    background: #1570a6
}
.row-blue,
.row-blue h1,
.row-blue h2,
.row-blue h3,
.row-blue h4,
.row-blue a,
.row-blue a:hover,
.row-blue a:focus {
    color: #fff
}
@media screen and (min-width: 540px) {
    .row-padded {
        padding-top: 2.5%;
        padding-bottom: 2.5%
    }
    .row-padded-top {
        padding-top: 2.5%
    }
    .row-padded-bottom {
        padding-bottom: 2.5%
    }
}
body.lt-ie9 {
    min-width: 1180px
}
.lt-ie9 .grid-container .col-flow-opposite {
    float: right
}
.lt-ie9 .row-padded {
    padding-top: 3em;
    padding-bottom: 3em
}
.lt-ie9 .row-padded-top {
    padding-top: 3em
}
.lt-ie9 .row-padded-bottom {
    padding-bottom: 3em
}
html[dir="rtl"].iem7,
html[dir="rtl"].iem9 {
    direction: ltr
}
html[dir="rtl"].iem7 form,
html[dir="rtl"].iem9 form {
    direction: rtl
}
html[dir="rtl"].iem7 .grid-row .grid-unit,
html[dir="rtl"].iem7 .grid-row .col-flow-opposite,
html[dir="rtl"].iem7 .grid-row .col-flow-opposite,
html[dir="rtl"].iem7 .grid-container .col-flow-opposite {
    float: none
}
html[dir="rtl"].iem7 .grid-row .grid-unit {
    clear: both
}
@media screen and (min-width: 900px) {
    .row--padded-right-1-4 .grid-row,
    .row--padded-right-1-4.grid-row {
        padding-right: 24.5%;
        width: 74.5%
    }
    .row--padded-left-1-4 .grid-row,
    .row--padded-left-1-4.grid-row {
        padding-left: 24.5%;
        width: 74.5%
    }
    .row--padded-right-1-3 .grid-row,
    .row--padded-right-1-3.grid-row {
        padding-right: 34%;
        width: 66%
    }
    .row--padded-left-1-3 .grid-row,
    .row--padded-left-1-3.grid-row {
        padding-left: 34%;
        width: 66%
    }
    .row--padded-right-1-2 .grid-row,
    .row--padded-right-1-2.grid-row {
        padding-right: 51%;
        width: 49%
    }
    .row--padded-left-1-2 .grid-row,
    .row--padded-left-1-2.grid-row {
        padding-left: 51%;
        width: 49%
    }
    .row--padded-right-2-3 .grid-row,
    .row--padded-right-2-3.grid-row {
        padding-right: 68%;
        width: 32%
    }
    .row--padded-left-2-3 .grid-row,
    .row--padded-left-2-3.grid-row {
        padding-left: 68%;
        width: 32%
    }
    .row--padded-right-3-4 .grid-row,
    .row--padded-right-3-4.grid-row {
        padding-right: 76.5%;
        width: 23.5%
    }
    .row--padded-left-3-4 .grid-row,
    .row--padded-left-3-4.grid-row {
        padding-left: 76.5%;
        width: 23.5%
    }
}
.background--right-top .grid-container {
    background-repeat: no-repeat;
    background-position: right top
}
.background--right-bottom .grid-container {
    background-repeat: no-repeat;
    background-position: right bottom
}
.background--center-top .grid-container {
    background-repeat: no-repeat;
    background-position: center top
}
.background--center-bottom .grid-container {
    background-repeat: no-repeat;
    background-position: center bottom
}
.background--left-bottom .grid-container {
    background-repeat: no-repeat;
    background-position: left bottom
}
.background--left-top .grid-container {
    background-repeat: no-repeat;
    background-position: left top
}
@media screen and (min-width: 0) and (max-width: 899px) {
    .background--hide-small .grid-container {
        background-image: none !important
    }
}
.hero {
    max-width: 1600px;
    margin: 0 auto;
    position: relative;
    overflow: hidden
}
.hero .slides {
    margin: 0;
    padding: 0;
    width: 100%;
    display: block
}
.hero .media {
    display: block;
    line-height: 0
}
.hero .media>a {
    display: block
}
.hero .text {
    overflow: hidden
}
.hero .nowrap {
    white-space: nowrap
}
.media {
    background-color: #f0f0f0;
    background-image: url("/CMSImages/progressIndicator40x40.gif?version=22bf30a9-5751-70e3-2c7d-7173410d2ed0");
    background-repeat: no-repeat;
    background-position: center center;
    line-height: 0;
    overflow: hidden
}
.media img,
.media video {
    max-width: 100%
}
.hero.hero-default-unmasked .media img,
.hero.hero-default-unmasked .media video {
    width: 100%;
    max-width: 100%;
    margin: 0 !important
}
@media screen and (min-width: 540px) {
    .hero .media img,
    .hero .media video {
        width: 100%;
        max-width: 100%;
        margin: 0
    }
}
@media screen and (min-width: 1025px) {
    .hero .media img,
    .hero .media video {
        margin-top: -1.875%
    }
}
@media screen and (min-width: 1025px) and (min-height: 900px) {
    .hero .media img,
    .hero .media video {
        margin: 0
    }
}
.hero.hero-med .media img,
.hero.hero-med .media video {
    width: 100%;
    max-width: 100%;
    margin: 0 !important
}
.hero.hero-full .media img,
.hero.hero-full .media video {
    width: 100%;
    max-width: 100%;
    margin: 0 !important
}
@media screen and (min-width: 680px) {
    .hero-video .box {
        -webkit-transition: opacity .2s ease;
        -moz-transition: opacity .2s ease;
        -o-transition: opacity .2s ease;
        -ms-transition: opacity .2s ease;
        transition: opacity .2s ease
    }
    .hero-video.playing .box {
        opacity: 0
    }
    .hero-video.playing:hover .hoverable .box,
    .hero-video.playing a:focus .hoverable .box {
        opacity: 1
    }
}
.slideshow-hero .navigation,
.slideshow-hero .prev-next {
    cursor: pointer;
    position: absolute;
    width: 100%
}
.slideshow-hero .prev-next .prev,
.slideshow-hero .prev-next .next {
    width: 0 !important;
    transition: all .5s
}
.slideshow-hero .prev-next.show .prev,
.slideshow-hero .prev-next.show .next {
    width: 30px !important
}
.slideshow-hero .navigation .container,
.slideshow-hero .prev-next .container {
    padding: 0 1%
}
.mscom-nonjs .slideshow-hero .navigation,
.mscom-nonjs .slideshow-hero .prev-next {
    display: none
}
.slideshow-hero .navigation {
    display: none;
    bottom: 1em;
    right: 1.15em;
    text-align: right;
    z-index: 960;
    pointer-events: none
}
.slideshow-hero .navigation a {
    color: #fff;
    display: -moz-inline-stack;
    display: inline-block;
    zoom: 1;
    *display: inline;
    font-size: 2.15em;
    margin: 0 .075em;
    position: relative;
    text-align: center;
    text-decoration: none;
    text-shadow: 0 0 4px #999;
    text-shadow: 0 0 4px rgba(0, 0, 0, .15);
    pointer-events: auto
}
.hp-hero-nav-gray .slideshow-hero .navigation a {
    color: #eee
}
.hp-hero-nav-dark-gray .slideshow-hero .navigation a {
    color: #ddd
}
.slideshow-hero .navigation a.active {
    color: #1570a6
}
.slideshow-hero .prev-next {
    bottom: 5.25em;
    z-index: 970
}
.slideshow-hero button {
    padding: 0;
    height: 2em;
    text-align: center;
    display: block;
    background: transparent;
    border: none;
    font-size: 1.5em;
    position: absolute;
    color: #1a1a1a;
    cursor: pointer;
    -moz-appearance: none;
    -webkit-appearance: none
}
.slideshow-hero .prev {
    text-align: left;
    left: 1%
}
.slideshow-hero .next {
    text-align: right;
    right: 1%
}
ul.slides>li {
    position: relative;
    -webkit-transition: opacity 1s ease-in-out;
    -moz-transition: opacity 1s ease-in-out;
    -o-transition: opacity 1s ease-in-out;
    transition: opacity 1s ease-in-out;
    z-index: 1
}
ul.slides>li.fade-out {
    opacity: 0;
    z-index: 0
}
ul.slides>li.fade-out .text {
    opacity: 0
}
ul.slides>li.fade-out .text .title {
    opacity: 0
}
ul.slides>li.hide {
    display: none !important
}
@media screen and (min-width: 33.75em) {
    .slideshow-hero .prev-next {
        bottom: 4.75em
    }
}
@media screen and (min-width: 42.5em) {
    .slideshow-hero .navigation {
        display: block
    }
    .slideshow-hero .prev-next {
        display: none
    }
}
.box {
    -webkit-box-sizing: border-box;
    -moz-box-sizing: border-box;
    box-sizing: border-box;
    background: #e55e1e;
    color: #fff;
    padding: 1em 40px 10px;
    min-height: 11.8em;
    position: relative;
    top: 0;
    font-size: .6em;
    cursor: pointer;
    z-index: 950
}
.box h3.box-title {
    display: block;
    font-size: 1.9em;
    line-height: 1em;
    margin: .25em 0 .35em 0;
    color: inherit
}
.box a:link,
.box a:visited,
.box a:hover,
.box a:active {
    color: inherit;
    cursor: pointer;
    pointer-events: auto
}
.box .box-description {
    margin-bottom: 0;
    line-height: 1;
    padding-bottom: 1em
}
.box li.box-description {
    font-size: 1.5em
}
li.box-actions {
    display: inline;
    line-height: 1.1em;
    margin-right: 4%
}
li.box-actions a:hover,
li.box-actions span:hover {
    text-decoration: underline
}
.box .box-img img {
    margin-bottom: .25em;
    max-height: 2.5em;
    width: auto
}
@media screen and (min-width: 33.75em) {
    .box {
        min-height: 10em;
        margin-left: auto
    }
}
@media screen and (min-width: 42.5em) {
    .hero .text {
        position: absolute;
        width: 100%;
        top: 20%;
        pointer-events: none
    }
    .hero .text-container {
        -webkit-box-sizing: border-box;
        -moz-box-sizing: border-box;
        box-sizing: border-box;
        width: 96%;
        max-width: 1180px;
        margin: 0 auto;
        text-align: left;
        cursor: pointer
    }
    .heroitem .text {
        cursor: pointer
    }
    .hero .box-right .text-container {
        text-align: right
    }
    .box {
        pointer-events: auto;
        text-align: left;
        display: -moz-inline-stack;
        display: inline-block;
        zoom: 1;
        *display: inline;
        font-size: .65em;
        width: 26em;
        padding: 1.2em 20px;
        min-height: 0;
        margin-left: 0
    }
    .box .box-img img {
        margin-bottom: 1.4em;
        margin-top: .4em;
        max-height: 2.1em
    }
    .box h3.box-title {
        line-height: 1.2em;
        margin: 0 0 2em 0
    }
    .box .box-description {
        padding-bottom: 0
    }
    .box li.box-description {
        font-size: 1.4em
    }
    .hero .text {
        top: 6%
    }
    .box li.box-actions {
        clear: both;
        display: block;
        margin-bottom: .5em
    }
}
@media screen and (min-width: 64.0625em) {
    .box {
        font-size: .8em;
        width: 23.4375em;
        bottom: 3em;
        padding: 1.4em 25px
    }
    .box .box-img img {
        max-height: 1.85em
    }
    .box h3.box-title {
        margin: 0 0 2.5em 0
    }
    .box li.box-description {
        font-size: 1.15em
    }
    li.box-actions {
        right: 1.4em;
        left: 1.4em
    }
    .hero .text {
        top: 7%
    }
}
@media screen and (min-width: 64.0625em) {
    .hero .text {
        top: 5%
    }
    .box h3.box-title {
        margin-bottom: 1.25em
    }
}
@media screen and (min-width: 64.0625em) and (min-height: 768px) {
    .hero .text {
        top: 6%
    }
    .box {
        font-size: .9em;
        width: 21.05263em
    }
    .box li.box-description {
        font-size: 1.1em
    }
    .box h3.box-title {
        margin-bottom: 2em
    }
    .box .box-img img {
        max-height: 1.6em
    }
}
.cssanimations .slideshow-hero {
    -webkit-animation: fadeIn;
    -moz-animation: fadeIn;
    -o-animation: fadeIn;
    animation: fadeIn;
    -webkit-animation-duration: .1s;
    -moz-animation-duration: .1s;
    -o-animation-duration: .1s;
    animation-duration: .1s
}
@media screen and (min-width: 680px) {
    .cssanimations .slideshow-hero .box,
    .cssanimations .video-fade-transition .box {
        position: relative;
        -webkit-animation: slideIn;
        -moz-animation: slideIn;
        -o-animation: slideIn;
        animation: slideIn;
        -webkit-animation-duration: 1s;
        -moz-animation-duration: 1s;
        -o-animation-duration: 1s;
        animation-duration: 1s
    }
}
.lt-ie9 .hero {
    zoom: 1
}
.lt-ie9 .hero .navigation {
    display: block
}
.lt-ie9 .hero .prev-next {
    bottom: 4.75em;
    display: none
}
.lt-ie9 .hero .media img {
    position: relative;
    left: 0;
    max-width: 100%;
    width: 100%;
    margin-left: 0;
    -ms-interpolation-mode: bicubic;
    margin-top: 0;
    margin-bottom: 0
}
.lt-ie9 .hero .text {
    position: absolute;
    clear: both;
    width: 100%;
    top: 3em
}
.lt-ie9 .hero .text {
    position: absolute;
    width: 100%;
    top: 20%
}
.lt-ie9 .hero .text-container {
    box-sizing: border-box;
    width: 1180px;
    margin: 0 auto;
    text-align: left
}
.lt-ie9 .hero .box-right .text-container {
    text-align: right
}
.lt-ie9 .box {
    text-align: left;
    display: -moz-inline-stack;
    display: inline-block;
    zoom: 1;
    *display: inline;
    font-size: .65em;
    width: 26em;
    padding: 1.2em 20px;
    min-height: 0;
    margin-left: 1%
}
.lt-ie9 .box .box-img img {
    margin-bottom: 1.4em;
    margin-top: .4em;
    height: 2.1em
}
.lt-ie9 .box h3.box-title {
    line-height: 1.2em;
    margin: 0 0 2em 0
}
.lt-ie9 .box .box-description {
    padding-bottom: 0
}
.lt-ie9 .hero .text {
    top: 6%
}
.lt-ie9 .box li.box-actions {
    clear: both;
    display: block;
    margin-bottom: .5em
}
.mscom-nonjs.lt-ie7 .hero .media {
    padding: 0
}
.iem7 .hero .prev-next button {
    width: 1.5em
}
@media screen and (min-width: 42.5em) {
    .NarrowBox .box {
        width: 21em
    }
    .ShiftBoxRight .box {
        margin-left: 4em;
        margin-top: .5em
    }
}
@media screen and (min-width: 53.5em) {
    .ShiftBoxRight .box {
        margin-left: 8em;
        margin-top: 3em
    }
}
@media screen and (min-width: 64.0625em) {
    .ShiftBoxRight .box {
        margin-left: 6em;
        margin-top: 1em
    }
}
@media screen and (min-width: 70em) {
    .ShiftBoxRight .box {
        margin-left: 8em;
        margin-top: 4em
    }
}
@media screen and (min-width: 64.0625em) and (min-height: 768px) {
    .ShiftBoxRight .box {
        margin-top: 1em
    }
}
@media screen and (min-width: 70em) and (min-height: 768px) {
    .ShiftBoxRight .box {
        margin-top: 2em
    }
}
@media screen and (min-width: 64.0625em) and (min-height: 900px) {
    .ShiftBoxRight .box {
        margin-top: 2em
    }
}
@media screen and (min-width: 70em) and (min-height: 900px) {
    .ShiftBoxRight .box {
        margin-top: 3.5em
    }
}
html,
body,
div,
span,
applet,
object,
iframe,
h1,
h2,
h3,
h4,
h5,
h6,
p,
blockquote,
pre,
a,
abbr,
acronym,
address,
big,
cite,
code,
del,
dfn,
em,
img,
ins,
kbd,
q,
s,
samp,
small,
strike,
strong,
sub,
sup,
tt,
var,
b,
u,
i,
center,
dl,
dt,
dd,
ol,
ul,
li,
fieldset,
form,
label,
legend,
table,
caption,
tbody,
tfoot,
thead,
tr,
th,
td,
article,
aside,
canvas,
details,
embed,
figure,
figcaption,
footer,
header,
hgroup,
menu,
nav,
output,
ruby,
section,
summary,
time,
mark,
audio,
video {
    margin: 0;
    padding: 0;
    border: 0;
    font-size: 100%;
    font: inherit;
    vertical-align: baseline
}
body {
    line-height: 1
}
ol,
ul {
    list-style: none
}
table {
    border-collapse: collapse;
    border-spacing: 0
}
caption,
th,
td {
    text-align: left;
    font-weight: normal;
    vertical-align: middle
}
q,
blockquote {
    quotes: none
}
q:before,
q:after,
blockquote:before,
blockquote:after {
    content: "";
    content: none
}
a img {
    border: none
}
article,
aside,
details,
figcaption,
figure,
footer,
header,
hgroup,
menu,
nav,
section,
summary {
    display: block
}
html {
    overflow-y: auto
}
img,
video {
    max-width: 100%;
    height: auto
}
.lt-ie9 img {
    -ms-interpolation-mode: bicubic;
    zoom: 1
}
@-webkit-keyframes fadeIn {
    0% {
        opacity: 0
    }
    100% {
        opacity: 1
    }
}
@-moz-keyframes fadeIn {
    0% {
        opacity: 0
    }
    100% {
        opacity: 1
    }
}
@-o-keyframes fadeIn {
    0% {
        opacity: 0;
    }
    100% {
        opacity: 1;
    }
}
@-webkit-keyframes fadeIn {
    0% {
        opacity: 0
    }
    100% {
        opacity: 1
    }
}
@-webkit-keyframes IE11FixfadeIn {
    0% {
        opacity: 1
    }
    100% {
        opacity: 1
    }
}
@-webkit-keyframes slideIn {
    0% {
        left: 3%;
        opacity: 0
    }
    100% {
        left: 0;
        opacity: 1
    }
}
@-moz-keyframes slideIn {
    0% {
        left: 3%;
        opacity: 0
    }
    100% {
        left: 0;
        opacity: 1
    }
}
@-o-keyframes slideIn {
    0% {
        left: 3%;
        opacity: 0;
    }
    100% {
        left: 0;
        opacity: 1;
    }
}
@-webkit-keyframes slideIn {
    0% {
        left: 3%;
        opacity: 0
    }
    100% {
        left: 0;
        opacity: 1
    }
}
@-webkit-keyframes slideIn {
    0% {
        left: 3%;
        opacity: 0
    }
    100% {
        left: 0;
        opacity: 1
    }
}
.list-of-links {
    float: none
}
.list-of-links li {
    display: inline-block;
    margin: .25em 0 .5em;
    width: 100%;
    line-height: 1.25em
}
.list-of-links-sm li,
.list-of-links-med li {
    font-size: .9em
}
.list-of-links-sm .heading {
    font-size: 1em;
    font-weight: normal;
    line-height: 1.25em;
    margin-bottom: .65em
}
.list-of-links-xlg .heading {
    margin-top: 0
}
@media screen and (min-width: 540px) {
    .list-array li {
        display: inline-block;
        margin: 0 2.5% 0 0;
        width: inherit
    }
    .list-of-links li:last-child {
        margin-right: 0
    }
    .list-of-links-xlg .heading {
        font-size: 2em
    }
}
@media screen and (min-width: 900px) {
    .list-of-links {
        margin-top: 0
    }
    .list-of-links ul {
        padding: .25em 0 .5em
    }
    .list-of-links li {
        display: block;
        margin: 0 0 .7em
    }
    .list-of-links-med .heading {
        font-size: 1.625em;
        margin-bottom: .55em
    }
    .ja-jp .list-of-links-med .heading {
        font-size: 1.2em
    }
    .list-of-links-lg .heading {
        font-size: 2.375em
    }
    .list-of-links-xlg li {
        font-size: 1.25em;
        font-family: wf_segoe-ui_light, 'Segoe UI Light', 'Segoe WP Light', wf_segoe-ui_normal, 'Segoe UI', Segoe, 'Segoe WP', Tahoma, Verdana, Arial, sans-serif;
        font-weight: 300
    }
    .list-of-links-xlg .heading {
        margin-bottom: .5em;
        font-size: 2.375em
    }
    .ja-jp .list-of-links-xlg .heading {
        font-size: 1.5em
    }
    .mac .list-of-links-xlg li {
        -webkit-font-smoothing: subpixel-antialiased
    }
}
.cssanimations .list-of-links-xlg {
    -webkit-animation: fadeIn;
    -moz-animation: fadeIn;
    -o-animation: fadeIn;
    animation: fadeIn;
    -webkit-animation-duration: 1s;
    -moz-animation-duration: 1s;
    -o-animation-duration: 1s;
    animation-duration: 1s
}
.cssanimations .list-of-links-xlg li {
    position: relative;
    -webkit-animation: slideIn;
    -moz-animation: slideIn;
    -o-animation: slideIn;
    animation: slideIn
}
.list-of-links-xlg li:nth-of-type(1) {
    -webkit-animation-duration: .225s;
    -moz-animation-duration: .225s;
    -o-animation-duration: .225s;
    animation-duration: .225s
}
.list-of-links-xlg li:nth-of-type(2) {
    -webkit-animation-duration: .45s;
    -moz-animation-duration: .45s;
    -o-animation-duration: .45s;
    animation-duration: .45s
}
.list-of-links-xlg li:nth-of-type(3) {
    -webkit-animation-duration: .675s;
    -moz-animation-duration: .675s;
    -o-animation-duration: .675s;
    animation-duration: .675s
}
.list-of-links-xlg li:nth-of-type(4) {
    -webkit-animation-duration: .9s;
    -moz-animation-duration: .9s;
    -o-animation-duration: .9s;
    animation-duration: .9s
}
.list-of-links-xlg li:nth-of-type(5) {
    -webkit-animation-duration: 1.125s;
    -moz-animation-duration: 1.125s;
    -o-animation-duration: 1.125s;
    animation-duration: 1.125s
}
.list-of-links-xlg li:nth-of-type(6) {
    -webkit-animation-duration: 1.35s;
    -moz-animation-duration: 1.35s;
    -o-animation-duration: 1.35s;
    animation-duration: 1.35s
}
.list-of-links-xlg li:nth-of-type(7) {
    -webkit-animation-duration: 1.575s;
    -moz-animation-duration: 1.575s;
    -o-animation-duration: 1.575s;
    animation-duration: 1.575s
}
.list-of-links-xlg li:nth-of-type(8) {
    -webkit-animation-duration: 1.8s;
    -moz-animation-duration: 1.8s;
    -o-animation-duration: 1.8s;
    animation-duration: 1.8s
}
.list-of-links-xlg li:nth-of-type(9) {
    -webkit-animation-duration: 2.025s;
    -moz-animation-duration: 2.025s;
    -o-animation-duration: 2.025s;
    animation-duration: 2.025s
}
.list-of-links-xlg li:nth-of-type(10) {
    -webkit-animation-duration: 2.25s;
    -moz-animation-duration: 2.25s;
    -o-animation-duration: 2.25s;
    animation-duration: 2.25s
}
.discover-list a {
    display: block;
    position: relative;
    padding-top: 52px;
    text-align: center;
    margin-top: .7em;
    margin-bottom: .7em
}
.discover-list img {
    position: absolute;
    top: 0;
    left: 0;
    padding-left: 50%;
    margin-left: -26px
}
@media screen and (max-width: 539px) {
    .discover-list {
        margin-top: .7em;
        margin-bottom: .7em
    }
}
.lt-ie9 .features img {
    width: 100%;
    margin: 0 0 .25em;
    float: none;
    padding-top: 0
}
.lt-ie9 .list-of-links {
    margin-top: 0
}
.lt-ie9 .list-of-links ul {
    padding: .25em 0 .5em
}
.lt-ie9 .list-of-links li {
    display: block;
    margin: 0 0 .7em
}
.lt-ie9 .list-of-links-med .heading {
    font-size: 1.625em;
    margin-bottom: .55em
}
.lt-ie9 .list-of-links-lg .heading {
    font-size: 2.375em
}
.lt-ie9 .list-of-links-xlg li {
    font-size: 1.25em;
    font-family: wf_segoe-ui_light, 'Segoe UI Light', 'Segoe WP Light', wf_segoe-ui_normal, 'Segoe UI', Segoe, 'Segoe WP', Tahoma, Verdana, Arial, sans-serif;
    font-weight: 300
}
.lt-ie9 .list-of-links-xlg .heading {
    margin-bottom: .5em;
    font-size: 2.375em
}
.CSPvNext .row-fluid .span .col-flow-opposite {
    float: right
}
.CSPvNext .row-fluid .span .col-flow-opposite+.span {
    margin-left: 0
}
.CSPvNext .mscom-remove-margin-left {
    margin-left: 0% !important;
    width: 100% !important
}
@media screen and (max-width: 539px) {
    .grid-container {
        width: 100%
    }
}
.CMSvNextComp .title,
.CMSvNextComp .title h3,
.CMSvNextComp .title a,
.CMSvNextComp .title a:hover,
.CMSvNextComp .title a:active,
.CMSvNextComp .title a:focus {
    color: #fff;
    text-decoration: none
}
.slideshow-news .slides a:hover,
.slideshow-news .slides a:active {
    text-decoration: none
}
.mscom-accordion {
    line-height: normal
}
.mscom-accordion .mscom-accordion-item {
    border-bottom: 1px solid #d2d2d2
}
.mscom-accordion .mscom-accordion-item-container {
    display: none;
    margin-bottom: 10px
}
.mscom-accordion .mscom-accordion-item-link {
    position: relative;
    display: block;
    padding: 12px 0
}
.mscom-accordion .mscom-accordion-item-link .mscom-accordion-item-image {
    position: absolute;
    height: 27px;
    width: 27px;
    display: inline-block;
    vertical-align: bottom;
    border: 1px solid transparent;
    overflow: hidden;
    left: 0;
    top: 50%;
    margin-top: -13.5px
}
.mscom-accordion .mscom-accordion-item-link .mscom-accordion-item-image:before {
    display: inline-block;
    content: url("CMSImages/accordion_icon_sprite.png?version=17099D97-CD5E-0507-9950-F316C697F104");
    margin: 0
}
.mscom-accordion .mscom-accordion-item-link:hover {
    text-decoration: none;
    background-color: #e6e6e6
}
.mscom-accordion .mscom-accordion-item-link:hover .mscom-accordion-item-image:before {
    margin-top: -37px
}
.mscom-accordion .mscom-accordion-item-link .mscom-accordion-item-title {
    display: inline-block;
    vertical-align: top;
    padding-left: 37px
}
.mscom-accordion .selected>.mscom-accordion-item-link .mscom-accordion-item-image:before {
    margin-top: -74px
}
.mscom-accordion .selected>.mscom-accordion-item-link:hover .mscom-accordion-item-image:before {
    margin-top: -111px
}
body.lt-ie9 .mscom-accordion .mscom-accordion-item-link .mscom-accordion-item-image {
    background: url("CMSImages/accordion_icon_sprite.png?version=17099D97-CD5E-0507-9950-F316C697F104");
    background-position: 0 0
}
body.lt-ie9 .mscom-accordion .mscom-accordion-item-link .mscom-accordion-item-image:before {
    content: none
}
body.lt-ie9 .mscom-accordion .mscom-accordion-item-link:hover .mscom-accordion-item-image {
    background-position: 0 -37px
}
body.lt-ie9 .mscom-accordion .selected>.mscom-accordion-item-link .mscom-accordion-item-image {
    background-position: 0 -74px
}
body.lt-ie9 .mscom-accordion .selected>.mscom-accordion-item-link:hover .mscom-accordion-item-image {
    background-position: 0 -111px
}
.mscom-popup .mscom-popup-link {
    color: #000;
    text-decoration: none;
    font-size: 90%
}
.mscom-popup .mscom-popup-container {
    display: none;
    position: fixed;
    height: 100%;
    width: 100%;
    z-index: 1002;
    top: 0;
    left: 0
}
.mscom-popup>.mscom-show-popup {
    display: table;
    *display: inline-block
}
.mscom-popup .mscom-popup-body {
    display: table-cell;
    vertical-align: middle;
    *display: inline-block;
    *top: 50%;
    *position: relative
}
.mscom-popup .mscom-popup-layout {
    width: 100%;
    *display: inline-block;
    *top: -50%;
    position: relative;
    z-index: 1003;
    max-height: 100vh;
    overflow: auto
}
.mscom-popup .mscom-popup-content-header {
    position: relative;
    padding: 20px 0
}
.mscom-popup .mscom-popup-content {
    margin: 0 auto;
    max-width: 1180px;
    background-color: #fff;
    border: none
}
.mscom-popup .mscom-popup-close {
    position: absolute;
    top: 10px;
    right: 10px
}
.mscom-popup .mscom-popup-close-image {
    height: 23px;
    width: 23px;
    *background-image: url("/CMSImages/CloseButtonGrey.png?version=A88C7E19-F369-4509-BEE8-3F74074D34A0");
    display: inline-block
}
.mscom-popup .mscom-popup-close-image:before {
    display: inline-block;
    content: url("/CMSImages/CloseButtonGrey.png?version=A88C7E19-F369-4509-BEE8-3F74074D34A0")
}
.mscom-popup .mscom-popup-mask {
    width: 100%;
    height: 100%;
    position: fixed;
    left: 0;
    top: 0;
    background-color: #ffff9b;
    opacity: 0;
    filter: alpha(opacity=0)
}
.mscom-popup .mscom-popup-maskcontent {
    background-color: #000;
    opacity: .85;
    filter: alpha(opacity=85)
}
.mscom-popup .mscom-progressbar {
    *background-image: url("/CMSImages/Loading.gif?version=D26CD9F0-5C36-D2DB-FB28-6B8F087E4E8B");
    background-repeat: no-repeat;
    background-position: center;
    height: 66px;
    padding: 10px 0;
    text-align: center
}
.mscom-popup .mscom-progressbar:before {
    display: inline-block;
    content: url("/CMSImages/Loading.gif?version=D26CD9F0-5C36-D2DB-FB28-6B8F087E4E8B")
}
.mscom-popup .mscom-popup-fullwidth>.mscom-popup-body>.mscom-popup-layout {
    background-color: #fff
}
.mscom-popup .mscom-popup-modal>.mscom-popup-body>.mscom-popup-layout {
    max-width: 1180px;
    margin: 0 auto
}
.mscom-popup .mscom-popup-modal>.mscom-popup-body>.mscom-popup-layout>.mscom-popup-content {
    border: 10px solid #969696
}
@media screen and (max-width: 679px) {
    .mscom-popup .mscom-popup-container {
        background-color: #fff
    }
    .mscom-popup .mscom-popup-body {
        vertical-align: top;
        height: 100%;
        overflow: auto
    }
    .mscom-popup .mscom-popup-content {
        max-width: 100%;
        border: none
    }
    .mscom-popup .mscom-popup-close {
        left: 10px
    }
    .mscom-popup .mscom-popup-close-image {
        *background-image: url("/CMSImages/LeftArrow_Grey.png?version=0BD21141-D55B-E8B9-18EF-F6AFA6676176")
    }
    .mscom-popup .mscom-popup-close-image:before {
        display: inline-block;
        content: url("/CMSImages/LeftArrow_Grey.png?version=0BD21141-D55B-E8B9-18EF-F6AFA6676176")
    }
    .mscom-popup .mscom-popup-mask {
        display: none
    }
    .mscom-popup .mscom-popup-modal>.mscom-popup-body>.mscom-popup-layout>.mscom-popup-content {
        max-width: 1180px;
        border: none
    }
}
.lt-ie9 .mscom-popup .mscom-popup-layout {
    max-height: 800px
}
body {
    font-size: 16px;
    line-height: 1.5;
    font-family: wf_segoe-ui_normal, 'Segoe UI', Segoe, 'Segoe WP', Tahoma, Verdana, Arial, sans-serif;
    font-weight: normal;
    -webkit-font-smoothing: antialiased;
    -webkit-text-size-adjust: 100%;
    -ms-text-size-adjust: 100%
}
p {
    margin: 0 0 1.5em
}
strong {
    font-weight: bold
}
em {
    font-style: italic
}
.legal {
    font-size: .833em
}
.screen-reader-text {
    border: 0;
    clip: rect(0 0 0 0);
    height: 1px;
    margin: -1px;
    overflow: hidden;
    padding: 0;
    position: absolute;
    width: 1px
}
input[type="text"],
input[type="password"],
input[type="email"],
input[type="search"],
input[type="submit"],
textarea {
    font-family: wf_segoe-ui_normal, 'Segoe UI', Segoe, 'Segoe WP', Tahoma, Verdana, Arial, sans-serif
}
h1,
h2,
h3,
h4 {
    font-family: wf_segoe-ui_light, 'Segoe UI Light', 'Segoe WP Light', wf_segoe-ui_normal, 'Segoe UI', Segoe, 'Segoe WP', Tahoma, Verdana, Arial, sans-serif;
    font-weight: 300;
    color: #1a1a1a;
    line-height: 1.25;
    margin: .25em 0 .75em;
    font-size: 1.825em
}
h5 {
    line-height: 1.25;
    margin: 0 0 .65em
}
.mac h1,
.mac h2,
.mac h3,
.mac h4 {
    -webkit-font-smoothing: subpixel-antialiased
}
h1 img,
h2 img,
h3 img,
h4 img,
h5 img,
h6 img {
    margin: 0
}
h1+h1,
h2+h2,
h3+h3 {
    margin-top: 1.3em
}
@media screen and (min-width: 540px) {
    h1 {
        margin-bottom: .35em
    }
    h2 {
        font-size: 1.625em;
        margin-bottom: .55em
    }
    h4 {
        font-size: 1.625em
    }
    h4 {
        font-size: 1.625em
    }
    h5 {
        font-size: 1em
    }
    h6 {
        font-size: 1em
    }
}
@media screen and (min-width: 680px) {
    h1 {
        font-size: 2em
    }
}
@media screen and (min-width: 900px) {
    h1 {
        font-size: 2.375em
    }
}
.CMSvNextComp h1,
.CMSvNextComp h2,
.CMSvNextComp h3,
.CMSvNextComp h4 {
    font-family: wf_segoe-ui_light, 'Segoe UI Light', 'Segoe WP Light', wf_segoe-ui_normal, 'Segoe UI', Segoe, 'Segoe WP', Tahoma, Verdana, Arial, sans-serif;
    font-weight: 300;
    color: #1a1a1a;
    line-height: 1.25;
    margin: .25em 0 .75em;
    font-size: 1.825em
}
.CMSvNextComp h5 {
    line-height: 1.25;
    margin: 0 0 .65em
}
.CMSvNextComp .mac h1,
.CMSvNextComp .mac h2,
.CMSvNextComp .mac h3,
.CMSvNextComp .mac h4 {
    -webkit-font-smoothing: subpixel-antialiased
}
.CMSvNextComp h1 img,
.CMSvNextComp h2 img,
.CMSvNextComp h3 img,
.CMSvNextComp h4 img,
.CMSvNextComp h5 img,
.CMSvNextComp h6 img {
    margin: 0
}
.CMSvNextComp h1+h1,
.CMSvNextComp h2+h2,
.CMSvNextComp h3+h3 {
    margin-top: 1.3em
}
@media screen and (min-width: 540px) {
    .CMSvNextComp h1 {
        margin-bottom: .35em
    }
    .CMSvNextComp h2 {
        font-size: 1.625em;
        margin-bottom: .55em
    }
    .CMSvNextComp h4 {
        font-size: 1.625em
    }
    .CMSvNextComp h4 {
        font-size: 1.625em
    }
    .CMSvNextComp h5 {
        font-size: 1em
    }
    .CMSvNextComp h6 {
        font-size: 1em
    }
}
@media screen and (min-width: 680px) {
    .CMSvNextComp h1 {
        font-size: 2em
    }
}
@media screen and (min-width: 900px) {
    .CMSvNextComp h1 {
        font-size: 2.375em
    }
}
a {
    color: #1570a6;
    text-decoration: none;
    -webkit-transition: color .1s linear;
    -moz-transition: color .1s linear;
    -ms-transition: color .1s linear;
    -o-transition: color .1s linear;
    transition: color .1s linear
}
a:focus,
a:hover {
    color: #083047;
    text-decoration: underline
}
.button {
    background-color: #1570a6;
    color: #fff;
    padding: .5em 1em;
    display: inline-block;
    -webkit-transition: background-color .3s ease-in, color .3s ease-in;
    -moz-transition: background-color .3s ease-in, color .3s ease-in;
    -ms-transition: background-color .3s ease-in, color .3s ease-in;
    -o-transition: background-color .3s ease-in, color .3s ease-in;
    transition: background-color .3s ease-in, color .3s ease-in
}
.button:hover,
.button:focus {
    background-color: #083047;
    color: #fff;
    text-decoration: none;
    -webkit-transition: all .08s ease-in;
    -moz-transition: all .08s ease-in;
    -ms-transition: all .08s ease-in;
    -o-transition: all .08s ease-in;
    transition: all .08s ease-in
}
blockquote {
    margin: 1.5em;
    color: #666;
    font-style: italic
}
dfn {
    font-style: italic;
    font-weight: bold;
    background: #ffc
}
sup,
sub {
    line-height: 0
}
abbr,
acronym {
    border-bottom: 1px dotted #666
}
address {
    margin: 0 0 1.5em;
    font-style: italic
}
del {
    color: #666
}
pre,
code {
    margin: 1.5em 0;
    white-space: pre
}
tt {
    display: block;
    margin: 1.5em 0;
    line-height: 1.5
}
pre,
code,
tt {
    font: 1em 'andale mono', 'lucida console', monospace;
    line-height: 1.5
}
.lt-ie9 h1 {
    font-size: 2.375em;
    margin-bottom: .5em
}
.lt-ie9 h2 {
    font-size: 1.625em
}
.lt-ie9 h3 {
    font-size: 1.167em
}
.lt-ie9 h4 {
    font-size: 1em
}
.lt-ie9 h5 {
    font-size: 1em
}
.lt-ie9 h6 {
    font-size: 1em
}
.international {
    font-family: Tahoma, Verdana, Arial, sans-serif
}
.mscom-video {
    position: relative;
    padding-bottom: 56.25%;
    padding-top: 0;
    height: 0;
    outline: none;
    overflow: hidden;
    width: auto !important
}
.mscom-video .vxplayer .slateContainer td {
    text-align: center
}
.mscom-video>div .vxp_content,
.mscom-video>div .vxpMultiLitePlayer {
    width: 100% !important
}
.mscom-video>div .vxp_richEmbedContainer {
    height: 0 !important;
    padding-bottom: 56.25% !important;
    width: 100% !important
}
.mscom-video>div .vxp_richEmbedContainer object {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%
}
.mscom-video .iframe iframe,
.mscom-video .iframe object,
.mscom-video .iframe embed {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%
}
.mscom-video .vxp_player .vxp_lead_with_image {
    height: auto !important;
    width: 100% !important
}
.lt-ie9 .mscom-popup .mscom-video {
    min-width: 1160px
}
#video-container {
    position: relative
}
.video-timebar-notice {
    background-color: #98afc7;
    border: 1px solid #777;
    color: #fff;
    display: none;
    opacity: 0;
    padding: 0 3px;
    transition: opacity .3s ease 0s;
    line-height: 1;
    height: 20px
}
.video-timebar:hover+.video-timebar-notice {
    border-radius: 8px;
    display: block;
    margin-top: -45px;
    opacity: 1;
    position: absolute
}
.video-timebar {
    background: -moz-linear-gradient(center top, #666 0%, #8e8e8e 50%, #666 100%) repeat scroll 0 0 rgba(0, 0, 0, 0);
    border: 1px solid #fff;
    height: 10px;
    width: 100%
}
.video-timebar-buffer {
    background-color: #8bb381;
    height: 10px;
    position: absolute;
    width: 0
}
.video-timebar:hover,
.video-volumebar {
    cursor: pointer
}
.video-timebar-inner {
    background: -moz-linear-gradient(center top, #6c9acc 0%, #008000 50%, #008000 100%) repeat scroll 0 0 rgba(0, 0, 0, 0);
    background-color: #008000;
    float: left;
    height: 10px;
    position: relative;
    width: 0
}
.video-timebar-pos {
    background-color: #444;
    border: 1px solid #fff;
    box-shadow: 0 0 5px #333;
    float: right;
    height: 10px;
    position: relative;
    right: -12px;
    top: -1px;
    width: 14px
}
.video-timebar:hover+.video-timebar-notice {
    border-radius: 8px;
    display: block;
    margin-top: -45px;
    opacity: 1;
    position: absolute
}
.video-container-overlay {
    position: absolute;
    background: #000;
    opacity: .9;
    top: 0;
    left: 0;
    z-index: 999
}
.video-timebar:hover,
.video-volumebar {
    cursor: pointer
}
.video-volume-ctrl {
    background-color: #262626;
    border: 1px solid #e3e3e3;
    color: #fff;
    display: none;
    opacity: 0;
    height: 110px;
    margin-top: -157px !important;
    margin-left: 55px !important;
    padding: 8px 2px 10px 7px !important;
    position: absolute;
    transition: opacity .3s ease 0s
}
.video-volumebar {
    background-color: #666;
    border: 1px solid #fff;
    border-radius: 7px;
    height: 100px;
    margin: 6px 6px 0 0 !important;
    position: relative;
    width: 12px
}
.video-volumebar-inner {
    background-color: #008000;
    border-radius: 7px;
    bottom: -1px;
    height: 75%;
    left: -1px;
    position: absolute;
    width: 14px
}
.video-volumebar-pos {
    background-color: #444;
    border: 1px solid #fff;
    border-radius: 14px;
    box-shadow: 0 0 5px #333;
    float: right;
    height: 14px;
    position: relative;
    right: -1px;
    top: -9px;
    width: 14px
}
.controls button:hover,
.controls button:focus,
.captions-menu li button:hover,
.captions-menu li button:focus,
.captions-menu li button[data-state="active"] {
    opacity: .5
}
#init {
    background: url("//c.s-microsoft.com//en-us/CMSImages/videobigplay.png?version=ddaaa84f-4a2b-a086-a846-b003e6e4547a") no-repeat scroll 50% 50% rgba(0, 0, 0, 0) !important;
    cursor: pointer;
    height: 100%;
    left: 0;
    position: absolute;
    top: 0;
    width: 100%;
    z-index: 2
}
.mute:hover+.video-volume-ctrl,
.video-volume-ctrl:hover,
.video-volume-ctrl:focus {
    display: block;
    opacity: 1
}
.captions-menu {
    display: none;
    position: absolute;
    border-radius: 10px;
    bottom: 14.8%;
    right: 20px;
    background: #666;
    list-style-type: none;
    margin: 0;
    padding: 0;
    width: auto;
    min-width: 110px;
    padding: 10px
}
.captions-menu li {
    padding: 0;
    text-align: center
}
.captions-menu li button {
    border: none;
    background: #000;
    color: #fff;
    cursor: pointer;
    width: 90%;
    padding: 2px 5px;
    -moz-border-radius: 2px;
    -webkit-border-radius: 2px;
    border-radius: 2px
}
.video-button-container button {
    width: 28px;
    height: 28px;
    background: url('https://c.s-microsoft.com/en-us/CMSImages/gl_site.svg?version=cb647d86-973f-f97b-1451-f52572ae7528') no-repeat;
    margin-left: 10px;
    margin-right: 10px
}
.video-button-container button:hover {
    opacity: .4
}
.video-button-container .video-facebook {
    width: 28px;
    height: 28px;
    background: url('https://cdn2.iconfinder.com/data/icons/New-Social-Media-Icon-Set-V11/24/facebook.png') no-repeat !important;
    margin-left: 10px;
    margin-right: 10px;
    background-size: 27px 27px !important
}
.video-button-container .video-twitter {
    width: 28px;
    height: 28px;
    background: url('https://cdn2.iconfinder.com/data/icons/New-Social-Media-Icon-Set-V11/24/twitter.png') no-repeat !important;
    margin-left: 10px;
    margin-right: 10px;
    background-size: 27px 27px !important
}
.video-button-container .play {
    background-position: 0 -1544px
}
.video-button-container .captions {
    background-position: 0 -732px
}
.video-button-container .video-light {
    background-position: 0 -964px
}
.video-button-container .fullscreen {
    background-position: 0 -1196px
}
.video-button-container .mute {
    background-position: 0 -1660px
}
#video-controls {
    position: absolute;
    margin-bottom: 10px;
    bottom: 0;
    left: 0;
    right: 0;
    height: 50px;
    opacity: 0;
    -webkit-transition: opacity .3s;
    -moz-transition: opacity .3s;
    -o-transition: opacity .3s;
    -ms-transition: opacity .3s;
    transition: opacity .3s;
    background-image: -webkit-gradient(linear, left bottom, left top, color-stop(.13, black), color-stop(1, black))
}
#video-container:hover #video-controls {
    opacity: .9
}
button {
    background: rgba(0, 0, 0, .5);
    border: 0;
    color: #eee;
    vertical-align: top;
    -webkit-border-radius: 3px;
    -moz-border-radius: 3px;
    -o-border-radius: 3px;
    border-radius: 3px
}
button:hover {
    cursor: pointer
}
video {
    width: 100%;
    height: 100%
}
@media screen and (max-width: 600px) {
    .video-light,
    .video-timer {
        display: none
    }
    .video-controls-right {
        width: 65% !important
    }
    .video-controls-left {
        width: 30% !important
    }
}
.mscom-youtube-video {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%
}
.grid.fixed.section {
    margin-bottom: 1.5em
}
.grid.fixed.section [class*="col-"] {
    padding-left: 10px;
    padding-right: 10px
}
.grid.fixed.section .section-header.row {
    position: relative
}
.grid.fixed.section .section-header.row .section-header-aside {
    position: absolute;
    top: .25em;
    right: 10px
}
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-prev,
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-next {
    position: static;
    cursor: pointer;
    background: none;
    top: 50%;
    margin-top: 0 0 0 .5em;
    padding: 0;
    border: none;
    font-size: 1.5em;
    outline: none;
    color: #1570a6;
    width: 1em;
    height: 1em;
    overflow: hidden
}
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-prev.slick-disabled,
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-next.slick-disabled {
    color: #d2d2d2;
    cursor: default
}
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-prev:before,
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-next:before {
    content: "";
    display: block;
    width: 1em;
    height: 1em;
    font-family: "Icons";
    font-style: normal;
    font-weight: normal;
    line-height: 1.1;
    -webkit-font-smoothing: antialiased;
    -moz-osx-font-smoothing: grayscale
}
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-prev {
    left: -1em
}
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-prev:before {
    background-image: url(data:image/png;base64,)
}
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-prev.slick-disabled:before {
    background-image: url(data:image/png;base64,)
}
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-next {
    right: -1em;
    margin: 0 0 0 .5em
}
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-next:before {
    background-image: url(data:image/png;base64,)
}
.grid.fixed.section .section-header.row .section-header-aside .section-header-arrows .slick-next.slick-disabled:before {
    background-image: url(data:image/png;base64,)
}
.grid.fixed.section .section-carousel .slick-list {
    overflow: hidden;
    outline: none;
    -webkit-transform: translate3d(0, 0, 0);
    -moz-transform: translate3d(0, 0, 0);
    -ms-transform: translate3d(0, 0, 0);
    transform: translate3d(0, 0, 0)
}
.grid.fixed.section .section-carousel .slick-list .slick-track {
    position: relative
}
.grid.fixed.section .section-carousel .slide {
    float: left
}
.grid.fixed.section .section-carousel .slide .media {
    background: none;
    margin: 0 10px;
    overflow: inherit
}
.grid.fixed.section .section-carousel .slide .media .media-img a {
    display: block;
    position: relative
}
.grid.fixed.section .section-carousel .slide .media .media-img a:after {
    content: "";
    position: absolute;
    top: 0;
    right: 0;
    bottom: 0;
    left: 0;
    background-color: rgba(255, 255, 255, 0);
    -webkit-transition: background-color .3s;
    -o-transition: background-color .3s;
    transition: background-color .3s
}
.grid.fixed.section .section-carousel .slide .media .media-img a:hover:after {
    background-color: rgba(255, 255, 255, .3)
}
.grid.fixed.section .section-carousel .slide .media .media-caption .media-header-clamp-2 {
    height: 2.75em;
    overflow: hidden;
    text-overflow: ellipsis
}
.grid.fixed.section .section-carousel .slide .media .media-caption .media-header {
    font-size: 16px;
    line-height: 1.375;
    font-family: "wf_segoe-ui_bold", "Arial", sans-serif;
    color: #000
}
.grid.fixed.section .section-carousel .slide .media .media-caption .media-header a {
    color: #1570a6
}
.grid.fixed.section .section-carousel .slide .media .media-caption .media-price {
    font-size: 13px;
    line-height: 1.53846;
    text-transform: capitalize;
    font-family: "wf_segoe-ui_bold", "Arial", sans-serif;
    font-weight: bold;
    color: #000
}
.grid.fixed.section .section-carousel .slide .media .media-caption .rating {
    display: inline-block;
    position: relative;
    margin-top: 8px
}
.grid.fixed.section .section-carousel .slide .media .media-caption .rating .StarsOverlay {
    position: absolute;
    overflow: hidden
}
.grid.fixed.section .section-carousel .slide .media .media-caption .rating img {
    min-width: 65px
}
.grid.fixed.section .section-carousel .slick-dots {
    margin: .5em 0 0;
    text-align: center
}
.grid.fixed.section .section-carousel .slick-dots li {
    display: inline-block
}
.grid.fixed.section .section-carousel .slick-dots li button {
    cursor: pointer;
    width: .8em;
    height: .8em;
    overflow: hidden;
    outline: none;
    line-height: 3;
    margin: 0 .15em;
    padding: 0;
    border: none;
    border-bottom: 2px solid #d2d2d2;
    background: none;
    border-radius: 0
}
.grid.fixed.section .section-carousel .slick-dots li.slick-active button {
    border-bottom-color: #1570a6
}
html,
body {
    overflow-x: hidden
}
p {
    font-family: Tahoma, Helvetica, Arial, "Microsoft Yahei","微软雅黑", STXihei, "华文细黑", sans-serif;
    font-size: 18px;
    color: #000;
    margin-bottom: 10px
}
h2 {
    font-size: 28px;
    margin-bottom: .3em;
    color: #000
}
b {
    font-weight: bold
}
i {
    font-style: italic
}
select {
    background-color: #0073c6;
    color: #fff;
    border: none;
    font-size: 20px;
    padding: 3px 30px 3px 10px;
    margin-bottom: 15px;
    max-width: 100%
}
select option {
    background-color: #fff;
    color: #000
}
sup {
    vertical-align: super;
    font-size: smaller
}
a.withArrow {
    margin-top: 5px;
    display: block;
    margin-bottom: 10px
}
a.withArrow img {
    width: 21px;
    height: 19px;
    vertical-align: text-top;
    margin-bottom: 0;
    padding-top: 2px;
    padding-right: 5px
}
p.withArrow img {
    width: 21px;
    height: 19px;
    vertical-align: text-top;
    margin-bottom: 0;
    padding-top: 2px;
    padding-right: 5px
}
span.withArrow img {
    width: 21px;
    height: 19px;
    vertical-align: text-top;
    margin-bottom: 0;
    padding-top: 2px;
    padding-right: 5px
}
a.withArrowRight img {
    width: initial;
    vertical-align: text-top;
    margin-bottom: 0;
    padding-top: 2px;
    padding-left: 3px
}
a.moreLink {
    color: #fff;
    background-color: #0073c6;
    font-size: 15px;
    padding: 5px 13px 6px 13px
}
h3.boldTitle {
    font-family: wf_segoe-ui_semibold, 'Segoe UI Light', 'Segoe WP Light', 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif !important;
    font-weight: 900 !important;
    font-size: 3em !important
}
ul.wpAndBgList {
    list-style: disc
}
ul.wpAndBgList li {
    margin-bottom: 10px
}
p.noBottomMargin {
    margin-bottom: 0 !important
}
span.underline {
    text-decoration: underline !important
}
span.sup {
    vertical-align: super;
    font-size: smaller
}
p span.seperate {
    display: block;
    margin-bottom: 25px
}
p.subhead {
    margin-bottom: 0
}
ul.first-after-subhead {
    margin-top: 0 !important
}
#ResourceContent {
    display: none !important
}
#ComplianceContent {
    display: none !important
}
.wider {
    max-width: 1300px;
    margin-right: auto;
    margin-left: auto
}
.grayback {
    background-color: #e6e6e6
}
.whitePaperService {
    color: #f00
}
ul#trusted-cloud-nav li a {
    font-size: .9em;
    padding: 0 10px
}
ul#trusted-cloud-nav>li.currentLocation a {
    color: #0081c6
}
ul#trusted-cloud-nav>li:hover {
    background-color: #dbddde
}
ul#trusted-cloud-nav>li.noSub:hover,
ul#trusted-cloud-nav>li[title="Whats-New"]:hover,
ul#trusted-cloud-nav>li[title="Home"]:hover,
ul#trusted-cloud-nav>li[title="STP"]:hover {
    background-color: #fff
}
.mscom-nav .mscom-nav-container {
    margin: 0 0 0 -10px
}
.mscom-navitem>div.subNavContainer {
    position: absolute;
    z-index: 100;
    width: auto !important;
    background-color: rgba(219, 221, 222, .95);
    display: table;
    padding: 10px 0 15px 25px;
    display: none
}
.mscom-navitem>div.subNavContainer>div {
    padding-right: 40px;
    width: auto !important;
    float: none !important;
    display: table-cell !important
}
.searchFilter {
    color: #f00 !important;
    font-size: 18px !important
}
.mobileHome {
    display: none
}
div.breadcrumbs a:link,
div.breadcrumbs a:hover,
div.breadcrumbs a:active,
div.breadcrumbs a:visited {
    color: #000
}
body>div.row-fluid:nth-of-type(3) div.row-fluid {
    margin-bottom: 30px
}
body>div.row-fluid:nth-of-type(3) div.row-fluid.hero {
    margin-bottom: 10px
}
body>div.row-fluid:nth-of-type(3) div.row-fluid.noBottomBorder {
    margin-bottom: 0
}
div.slideshow-hero ul.slides li div.heroitem div.text {
    height: 100%;
    top: 0
}
div.slideshow-hero ul.slides li div.heroitem div.text div.text-container {
    width: 100%;
    margin: 0;
    height: 100%
}
div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box {
    height: 100%;
    width: 44%;
    padding: 0 5%
}
div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box ul {
    margin-top: 40px
}
div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box h3 {
    font-size: 2.3em;
    font-family: wf_segoe-ui_light, 'Segoe UI Light', 'Segoe WP Light', 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif;
    color: #fff;
    margin: 0
}
div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box p {
    font-size: 21px;
    margin-bottom: 0;
    margin-top: 18px;
    color: #fff;
    line-height: 27px
}
.CSPvNext .row-fluid.mscom-grid-container,
.CSPvNext .mscom-grid-container {
    width: 90%
}
#Services div.image-wrapper,
#Services2 div.image-wrapper {
    height: 43px
}
#Services a.moreLink,
#Services2 a.moreLink {
    color: #fff;
    background-color: #0073c6;
    font-size: 15px;
    padding: 5px 13px 6px 13px
}
#Services p,
#Services2 p {
    font-size: 15px;
    width: 215px;
    max-width: 100%;
    color: #000
}
#Services p a.withArrow,
#Services2 p a.withArrow {
    display: inline !important
}
#Services div.image-wrapper img.adjusted,
#Services2 div.image-wrapper img.adjusted {
    position: relative;
    left: -2px
}
.mscom-accordion .mscom-accordion-item-link .mscom-accordion-item-image:before {
    content: url("/en-us/CMSImages/accordion_icon_sprite.png?version=e508c2ef-3d89-adbc-665d-0fbf5d4988fc&CollectionId=e1e48768-8566-414a-9e99-ee683e406dde")
}
.mscom-accordion .mscom-accordion-item-link .mscom-accordion-item-image {
    margin-top: -9.5px;
    top: 20px
}
.mscom-accordion .mscom-accordion-item-link {
    padding: 12px 0 1px 0
}
.features {
    padding-top: 30px;
    padding-bottom: 15px
}
.features h3 {
    margin: 0;
    font-size: 26px;
    font-family: wf_segoe-ui_light, 'Segoe UI Light', 'Segoe WP Light', 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif;
    color: #000;
    margin-top: 10px
}
.features p {
    margin: 0;
    font-size: 15px;
    color: #000
}
.features .features-icon {
    width: auto !important
}
.features .features-icon img {
    width: 75px
}
.features>div>div.row-fluid>div:nth-of-type(2) {
    width: 130px;
    max-width: 49%
}
.features .span {
    margin-bottom: 0
}
.lightParagraph p {
    color: #000
}
.whatsNew {
    background-color: #0073c6
}
.whatsNew .right {
    color: #fff
}
.whatsNew .left img {
    width: 100%
}
.whatsNew h1 {
    color: #fff;
    font-size: 54px;
    margin-bottom: 5px !important
}
.whatsNew h3 {
    color: #fff;
    font-size: 28px;
    margin-bottom: 0;
    font-family: wf_segoe-ui_normal, 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif
}
.whatsNew p {
    font-size: 17px;
    padding-right: 125px;
    color: #fff
}
.whatsNew a {
    color: #fff;
    text-decoration: underline
}
.whatsNew a.withArrowRight {
    text-decoration: none
}
.whatsNewDeDe p {
    font-size: 15px
}
.title1Adjustment {
    margin-top: 2px
}
.title2Adjustment {
    margin-top: 4px
}
.title3Adjustment {
    margin-top: 2px
}
div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box ul.subpageHeaderCaption h3 {
    font-size: 38px
}
#video-container {
    margin-top: 10px
}
.subpageBody {
    max-width: 1300px;
    margin-right: auto;
    margin-left: auto
}
.subpageBody>div:first-of-type {
    width: 66% !important;
    padding-left: 60px
}
.subpageBody>div:first-of-type div.row-fluid label,
.subpageMain>div:first-of-type div.row-fluid label,
.subpageMain>div:first-of-type label,
.subpageBody>div:first-of-type label {
    font-family: wf_segoe-ui_light, 'Segoe UI Light', 'Segoe WP Light', 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif;
    font-size: 30px;
    line-height: 36px
}
.subpageBody>div:first-of-type div.row-fluid label.subhead,
.subpageMain>div:first-of-type div.row-fluid label.subhead,
.subpageMain>div:first-of-type label.subhead,
.subpageBody>div:first-of-type label.subhead {
    font-size: 20px !important
}
.subpageBody>div:first-of-type div.row-fluid label.selectLabel,
.subpageMain>div:first-of-type div.row-fluid label.selectLabel,
.subpageMain>div:first-of-type label.selectLabel,
.subpageBody>div:first-of-type label.selectLabel {
    font-size: 16px !important;
    font-family: wf_segoe-ui_normal, 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif !important
}
.subpageBody>div:first-of-type div.row-fluid p,
.subpageMain>div:first-of-type div.row-fluid p {
    color: #000;
    margin-bottom: 10px
}
.subpageBody>div:first-of-type div.row-fluid p a.withArrow {
    display: inline-block;
    font-size: 18px;
    text-decoration: none;
    font-family: wf_segoe-ui_normal, 'Segoe UI', Segoe, 'Segoe WP', Tahoma, Arial, sans-serif;
    margin-bottom: 0
}
.subpageBody>div:first-of-type div.row-fluid img.complianceImg {
    float: left;
    width: 33%
}
.subpageBody>div:first-of-type div.row-fluid a.withArrow {
    font-family: wf_segoe-ui_light, 'Segoe UI Light', 'Segoe WP Light', 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif;
    font-size: 17px
}
.subpageBody>div:first-of-type div.row-fluid a.withArrow.normal {
    font-size: 18px;
    color: auto;
    text-decoration: none;
    font-family: wf_segoe-ui_normal, 'Segoe UI', Segoe, 'Segoe WP', Tahoma, Arial, sans-serif
}
.subpageBody>div:first-of-type div.row-fluid div.mscom-accordion>ul {
    list-style-type: none;
    margin-left: 0
}
.subpageBody>div:first-of-type div.row-fluid div.mscom-accordion>ul a {
    color: #1570a6
}
.subpageBody>div:first-of-type div.row-fluid div.mscom-accordion>ul>li>a {
    color: #1570a6;
    text-decoration: none;
    font-weight: 600
}
.subpageBody>div:first-of-type div.row-fluid div.mscom-accordion>ul>li>a:nth-of-type(1) {
    font-weight: bold
}
.subpageBody>div:first-of-type div.row-fluid li.mscom-accordion-item {
    border-bottom: none;
    color: #000
}
.subpageBody>div:first-of-type div.row-fluid li.mscom-accordion-item:first-of-type {
    float: right;
    top: -50px;
    position: relative;
    height: 0
}
.subpageBody>div:first-of-type div.row-fluid li.mscom-accordion-item a {
    color: #000;
    font-size: 18px !important
}
.subpageBody>div:first-of-type div.row-fluid li.mscom-accordion-item a:hover {
    background-color: #fff
}
.subpageBody>div:first-of-type div.row-fluid li.mscom-accordion-item div.mscom-accordion-item-container {
    margin-left: 38px
}
.subpageBody>div:nth-of-type(2) {
    width: 32% !important;
    word-break: break-word
}
.subpageBody>div:nth-of-type(2)>div {
    margin-bottom: 20px !important
}
.subpageBody>div:nth-of-type(2) div.borderBottom {
    border-bottom: solid 8px #0072c6
}
.subpageBody>div:nth-of-type(2) p,
.subpageBody>div:nth-of-type(2) label,
.subpageBody>div:nth-of-type(2) a {
    margin-left: 30px;
    margin-right: 30px;
    display: inline-block;
    font-size: 16px
}
.subpageBody>div:nth-of-type(2) h1 {
    color: #fff;
    background-color: #0072c6;
    font-family: wf_segoe-ui_normal, 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif;
    font-size: 28px;
    font-weight: bold;
    padding: 15px 30px
}
.subpageBody>div:nth-of-type(2) label {
    font-size: 18px;
    font-weight: bold;
    font-family: wf_segoe-ui_normal, 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif;
    color: #000
}
.subpageBody>div:nth-of-type(2) label a {
    margin-right: 0;
    margin-left: 0;
    font-size: 18px;
    margin-bottom: 12px
}
.subpageBody>div:nth-of-type(2) label.no-space a {
    margin-bottom: 0
}
.subpageBody>div:nth-of-type(2) label.withTail {
    margin-bottom: 12px
}
.subpageBody>div:nth-of-type(2) label.withTail span.tail {
    font-weight: normal
}
.subpageBody>div:nth-of-type(2) label.withTail a {
    margin-bottom: 0;
    display: inline
}
.subpageBody>div:nth-of-type(2) label a.withParagraph {
    margin-bottom: 0
}
.subpageBody>div:nth-of-type(2) label a:focus {
    color: #551a8b
}
.subpageBody>div:nth-of-type(2) label.side-bar-link {
    margin-bottom: 12px
}
.subpageBody>div:nth-of-type(2) label.side-bar-link span,
.subpageBody>div:nth-of-type(2) li.side-bar-link span {
    font-size: 16px;
    font-weight: normal
}
.subpageBody>div:nth-of-type(2) label.side-bar-link a,
.subpageBody>div:nth-of-type(2) li.side-bar-link a {
    display: inline
}
.subpageBody>div:nth-of-type(2) p {
    margin-bottom: 0;
    margin-bottom: 12px;
    color: #000
}
.subpageBody>div:nth-of-type(2) p a {
    margin-left: 0;
    margin-right: 0;
    display: inline
}
.subpageBody>div:nth-of-type(2) a.withArrow {
    color: #000;
    font-size: 20px;
    font-weight: bold;
    margin-bottom: 20px
}
.subpageBody>div:nth-of-type(2) a.withArrow img {
    padding-top: 6px;
    padding-right: 14px
}
.subpageBody>div:nth-of-type(2) span.withArrow img {
    padding-top: 6px;
    padding-right: 14px
}
.subpageBody>div:nth-of-type(2) ul li a.withArrow {
    margin-bottom: 0
}
.subpageBody>div:nth-of-type(2) ul li.withTail a {
    display: inline
}
.subpageBody>div:nth-of-type(2) span.withTail {
    margin-bottom: 20px;
    display: block;
    margin-left: 30px;
    margin-right: 30px;
    color: #000;
    font-size: 20px;
    font-weight: bold
}
.subpageBody>div:nth-of-type(2) span.withTail a.withArrow {
    margin-bottom: 0;
    display: inline;
    margin-left: 0;
    margin-right: 0
}
.subpageBody>div:nth-of-type(2) span.withTail span.tail {
    font-weight: normal
}
.subpageBody>div:first-of-type .complianceDescription>div:first-of-type {
    width: 30%
}
.subpageBody>div:first-of-type .complianceDescription>div:first-of-type img {
    width: 100%
}
.subpageBody>div:first-of-type .complianceDescription>div:nth-of-type(2) {
    width: 68%
}
.subpageBody>div:first-of-type ul,
.subpageMain>div:first-of-type ul {
    list-style-type: disc;
    margin: 10px 0 10px 30px
}
.subpageBody>div:first-of-type ul li,
.subpageMain>div:first-of-type ul li {
    margin-bottom: 10px;
    font-size: 18px
}
.subpageBody>div:first-of-type ul li span,
.subpageBody>div:nth-of-type(2) ul li span {
    color: #000
}
.subpageBody>div:nth-of-type(2) ul {
    list-style-type: disc;
    margin-left: 30px;
    padding-left: 30px;
    margin-bottom: 12px
}
.subpageBody>div:nth-of-type(2) ul li a {
    margin-left: 0;
    margin-right: 0;
    vertical-align: top
}
.subpageBody>div:nth-of-type(2) ol {
    list-style-type: decimal;
    margin-left: 30px;
    margin-right: 30px;
    margin-bottom: 12px
}
.subpageBody>div:nth-of-type(2) ol li {
    margin-left: 30px;
    color: #000
}
.subpageBody>div:nth-of-type(2) ol li a {
    margin-left: 0;
    margin-right: 0
}
.subpageBody>div:nth-of-type(2) iframe {
    width: 100%;
    height: 232px;
    margin-top: 10px
}
.subpageBody>div:nth-of-type(2) .compliantServices>div:first-of-type {
    max-width: 206px
}
.subpageBody>div:nth-of-type(2) .compliantServices>div:nth-of-type(2) {
    max-width: 206px;
    margin-left: 4px
}
.subpageBody>div:nth-of-type(2) .compliantServices>div div {
    display: inline-block;
    width: 100%;
    background-color: #000;
    color: #fff;
    font-family: wf_segoe-ui_normal, 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif;
    font-size: 18px;
    padding-top: 5px;
    padding-bottom: 5px;
    margin-top: 2px;
    margin-bottom: 2px
}
.subpageBody>div:nth-of-type(2) .compliantServices>div div span {
    padding-left: 15px;
    padding-right: 15px;
    display: block
}
.subpageBody>div:first-of-type div.row-fluid div.mscom-accordion>ul a {
    font-size: 18px
}
.subpageBody>div:first-of-type div.mscom-accordion>ul.nonFAQ {
    list-style-type: none;
    margin-left: 0
}
.subpageBody>div:first-of-type div.mscom-accordion>ul.nonFAQ a {
    color: #1570a6;
    text-decoration: none;
    font-weight: 600
}
.subpageBody>div:first-of-type div.mscom-accordion>ul.nonFAQ a:nth-of-type(1) {
    font-weight: bold
}
.subpageBody>div:first-of-type div.row-fluid li.mscom-accordion-item ul.nonFAQ li.mscom-accordion-item:first-of-type {
    float: none;
    height: auto;
    top: 0
}
.subpageBody>div:first-of-type li.mscom-accordion-item {
    border-bottom: none;
    color: #000
}
.subpageBody>div:first-of-type li.mscom-accordion-item a {
    color: #000;
    font-size: 18px !important
}
.subpageBody>div:first-of-type li.mscom-accordion-item a:hover {
    background-color: #fff
}
.subpageBody>div:first-of-type li.mscom-accordion-item div.mscom-accordion-item-container {
    margin-left: 38px
}
.subpageBody>div:first-of-type div.complianceSort {
    display: block
}
table {
    border-top: 2px solid #c9c9c9;
    border-spacing: 0;
    border-collapse: collapse;
    padding: 0;
    text-align: left;
    width: 100%;
    font-size: 16px
}
table thead {
    border-bottom: 2px solid #dedede
}
table thead th {
    width: 50%;
    padding: 10px;
    min-width: 50px;
    text-transform: uppercase;
    vertical-align: top;
    color: #000;
    font-weight: 700
}
table tbody tr {
    border-bottom: 1px solid #dedede
}
table tbody tr td {
    padding: 10px;
    vertical-align: top;
    color: #000
}
table.tblCompliance {
    border: none;
    margin-top: 20px;
    margin-bottom: 20px
}
table.tblCompliance tbody tr {
    border: none
}
table.tblCompliance tbody tr:first-of-type {
    background-color: #0072c6
}
table.tblCompliance tbody tr:first-of-type td {
    color: #fff;
    font-weight: bold
}
table.tblCompliance tbody tr:nth-of-type(2n+3) td:first-of-type,
table.tblCompliance tbody tr:nth-of-type(2n+3) td:nth-of-type(3) {
    background-color: #e6e6e6
}
table.tblCompliance tbody tr:nth-of-type(n+2) td:first-of-type span {
    max-width: 300px;
    padding-right: 75px;
    display: block
}
table.tblCompliance tbody tr:nth-of-type(n+2) td:nth-of-type(3) span {
    max-width: 375px;
    display: block
}
table.tblCompliance tbody tr td:nth-of-type(2) {
    width: 10px;
    margin: 0;
    padding-left: 0;
    padding-right: 0
}
table.tblCompliance tbody tr:nth-of-type(n+2) td {
    vertical-align: middle
}
div.whatsNewBody>div>div.row-fluid {
    margin-bottom: 0 !important;
    padding-top: 15px;
    padding-bottom: 15px
}
div.whatsNewBody>div>div.row-fluid:nth-of-type(n+2) {
    border-top: 3px solid #0072c6
}
div.whatsNewBody>div>div.row-fluid:nth-of-type(2n) {
    background-color: #e6e6e6
}
div.whatsNewBody>div>div.row-fluid>div>div.row-fluid {
    margin-bottom: 0 !important
}
div.whatsNewBody>div>div.row-fluid>div>div.row-fluid label.date {
    font-size: 16px;
    color: #0072c6;
    font-weight: bold
}
div.whatsNewBody>div>div.row-fluid>div>div.row-fluid label a {
    font-family: wf_segoe-ui_light, 'Segoe UI Light', 'Segoe WP Light', 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif;
    font-size: 30px;
    line-height: 36px;
    color: #000
}
div.whatsNewBody>div>div.row-fluid>div>div.row-fluid img {
    float: right;
    margin-left: 15px
}
div.certImageContainer,
div.resourceImageTable {
    position: relative
}
div.certImageContainer div.certImageColor {
    position: absolute;
    z-index: 1
}
div.certImageContainer div.certImageGray {
    z-index: 10;
    position: relative
}
div.certImageContainer div.certImageGray img {
    cursor: pointer
}
div.certImageContainer div.certImageGray img:hover,
div.certImageContainer div.certImageGray img.active,
div.resourceImageTable div.certImageGray img:hover,
div.resourceImageTable div.certImageGray img.active {
    opacity: 0
}
div.certImageContainer div.imageWrapper {
    display: inline-block;
    width: 153px;
    text-align: center;
    margin-left: 4px;
    margin-right: 4px;
    vertical-align: middle;
    height: 154px
}
div.resourceImageTable div.imageWrapper {
    display: inline-block;
    width: 153px;
    text-align: center;
    margin-left: 5px;
    margin-right: 5px;
    vertical-align: middle;
    height: 154px
}
div.certImageTable div.resourceImgs img {
    cursor: pointer
}
div.resourceImageTable>div:first-of-type {
    width: 62% !important;
    position: relative;
    min-height: 560px;
    border-bottom: 2px solid #dbddde
}
div.resourceImageTable>div:nth-of-type(2) {
    border-left: solid 5px #0072c6;
    min-height: 560px;
    width: 38% !important;
    position: absolute;
    right: 0;
    height: 100% !important;
    padding-left: 30px;
    padding-right: 30px;
    margin-left: 0
}
div.resourceImageTable>div:nth-of-type(2)>div>a {
    display: none;
    font-size: 20px
}
div.resourceImageTable>div:nth-of-type(2) img {
    display: none;
    width: 130px;
    left: -30px;
    margin-top: 15px;
    position: relative
}
div.resourceImageTable>div:nth-of-type(2) h1 {
    font-weight: 900
}
.certDescriptionArea {
    border-top: solid 5px #0072c6;
    min-height: 350px
}
.certDescriptionArea>div {
    max-width: 1180px;
    margin-left: auto !important;
    margin-right: auto;
    float: none !important
}
.certDescriptionArea h1 {
    font-size: 18px;
    font-weight: bold;
    font-family: wf_segoe-ui_normal, 'Segoe UI', 'Segoe WP', Tahoma, Arial, sans-serif;
    margin-top: 20px
}
.certDescriptionArea a {
    display: none
}
div.certDescriptionArea div.certDescription>div>div:nth-of-type(2) {
    width: auto;
    max-width: 1000px
}
div.certDescriptionArea div.certDescription>div>div:first-of-type {
    width: 153px
}
div.certDescriptionArea div.certDescription img {
    display: none
}
div.certPopup {
    background-color: #0072c6;
    position: absolute;
    z-index: 100;
    padding-bottom: 30px
}
div.certPopup h1 {
    background-color: none
}
div.certPopup p {
    color: #fff !important;
    padding-bottom: 15px
}
div.certPopup a {
    color: #fff !important
}
div.certPopup>img {
    position: absolute;
    right: 0;
    top: 0;
    margin: 10px;
    cursor: pointer
}
.certSeeMore {
    float: right
}
div.certImageTable div.certImageContainer div.certPopup {
    background-color: #fff;
    box-shadow: 5px 5px 13px #c2c2c2;
    width: 500px
}
div.certImageTable div.certImageContainer div.certPopup h1,
div.certImageTable div.certImageContainer div.certPopup p {
    padding-left: 30px;
    padding-right: 30px;
    color: #000 !important
}
div.certImageTable div.certImageContainer div.certPopup>a {
    padding-left: 30px;
    padding-right: 30px;
    color: #0072c6 !important
}
div.certImageTable div.certImageContainer div.certPopup p a {
    color: #0072c6 !important;
    display: inline !important
}
div.certImageTable div.certImageContainer div.certPopup p span.withTail {
    display: block
}
div.certImageTable div.certImageContainer div.certPopup p span.withTail a {
    margin-bottom: 0;
    display: inline !important;
    margin-left: 0;
    margin-right: 0
}
@media(min-width:801px) {
    .showMobile {
        display: none
    }
    .hideMobile {
        display: block
    }
}
@media(max-width:800px) and (min-width:481px) {
    .showMobile {
        display: none
    }
    .hideMobile {
        display: block
    }
    div.slideshow-hero ul.slides li div.heroitem div.text {
        height: auto
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container {
        height: auto
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box {
        height: auto;
        width: 100%;
        padding-top: 20px;
        padding-bottom: 30px
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box ul {
        margin-top: 0
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box p {
        font-size: 15px
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box ul.subpageHeaderCaption h3 {
        font-size: 30px
    }
    .light-foreground .icon-left,
    .light-foreground .icon-right {
        display: none
    }
    .hero .text {
        position: relative
    }
    .subpageBody>div:first-of-type {
        width: 61% !important;
        padding-left: 40px
    }
    .subpageBody>div:nth-of-type(2) {
        margin-right: 5%
    }
    .subpageBody>div:nth-of-type(2) iframe {
        height: 140px
    }
    .subpageBody>div:nth-of-type(2) a.withArrow {
        font-size: 18px
    }
    div.certPopup {
        width: 400px
    }
    div.certDescriptionArea div.certDescription>div>div:first-of-type {
        margin-left: 40px
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box h3 {
        font-size: 26px
    }
    .box h3.box-title {
        font-size: 55px !important
    }
    .features>div,
    #Services>div,
    #Services2>div {
        width: 48% !important;
        margin-left: 2% !important
    }
    .features>div {
        height: 180px
    }
    .whatsNew h1 {
        font-size: 35px
    }
    .whatsNew p {
        font-size: 15px
    }
    #Services a.moreLink,
    #Services2 a.moreLink {
        display: inline-block;
        margin-bottom: 25px
    }
    .CSPvNext .row-fluid [class*="bp0-col"] {
        display: block
    }
    .mscom-header-nav-section.mscom-fullwidth-view-1 {
        display: none
    }
    .mobileHome {
        display: block
    }
    .mscom-nav .mscom-navitem,
    .mscom-nav .mscom-nav-item-link,
    .mscom-nav .mscom-nav-flyout .mscom-nav-layout li a,
    .mscom-nav .mscom-nav-flyout .mscom-nav-linklayout-horizontal ul li {
        display: block
    }
    div#trusted-cloud-nav {
        position: absolute;
        z-index: 1000;
        background-color: #fff;
        width: 300px;
        right: -300px;
        bottom: 0;
        top: 135px;
        overflow-y: scroll;
        box-shadow: 3px 3px 15px #a1a1a1
    }
    div#trusted-cloud-nav>ul {
        margin: 0
    }
    div#trusted-cloud-nav>ul li a {
        color: #000
    }
    .mscom-navitem>div.subNavContainer {
        position: relative;
        background-color: #fff
    }
    .mscom-navitem>div.subNavContainer>div {
        display: block !important;
        margin-left: 0
    }
    .mscom-header-row-2>div:first-of-type {
        width: 83% !important
    }
    .mscom-header-row-2>div:nth-of-type(2) {
        width: 15% !important
    }
    .mscom-header-search-section {
        width: 99% !important;
        margin: 0 !important
    }
    .mscom-header-search-section>div.mscom-header-section-2-4 {
        width: 100% !important
    }
    .mscom-header-search-section {
        display: none !important
    }
    .subpageBody>div:first-of-type div.row-fluid div.mscom-accordion>ul {
        position: relative
    }
    .subpageBody>div:first-of-type div.row-fluid li.mscom-accordion-item:first-of-type {
        float: none;
        top: 0;
        right: 0;
        position: absolute
    }
    .subpageBody>div:first-of-type div.row-fluid li.mscom-accordion-item:nth-of-type(2) {
        padding-top: 40px
    }
    .subpageBody>div:nth-of-type(2) {
        clear: none !important
    }
}
@media(max-width:600px) and (min-width:481px) {
    .showMobile {
        display: block !important
    }
    .hideMobile {
        display: none !important
    }
    div.slideshow-hero ul.slides li div.heroitem div.text {
        height: auto
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container {
        height: auto
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box {
        height: auto;
        width: 100%;
        padding-top: 20px;
        padding-bottom: 30px
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box ul {
        margin-top: 0
    }
    .subpageBody>div:first-of-type {
        padding-right: 60px;
        padding-left: 60px
    }
    .subpageBody>div:nth-of-type(2) {
        width: 100% !important
    }
    .subpageBody>div:first-of-type {
        width: 100% !important
    }
    .light-foreground .icon-left,
    .light-foreground .icon-right {
        display: none
    }
    .mscom-nav .mscom-nav-container {
        background-color: #fff
    }
    div#trusted-cloud-nav {
        position: absolute;
        z-index: 1000;
        background-color: #fff;
        width: 300px;
        right: -300px;
        bottom: 0;
        top: 135px;
        overflow-y: scroll;
        box-shadow: 3px 3px 15px #a1a1a1
    }
    div#trusted-cloud-nav>ul {
        margin: 0
    }
    div#trusted-cloud-nav>ul li a {
        color: #000
    }
    div#trusted-cloud-nav>ul>li>a {
        line-height: 2.5em
    }
    div#trusted-cloud-nav>ul>li>a:hover {
        background-color: #a1a1a1
    }
    div#trusted-cloud-nav>ul>li ul li a:hover {
        text-decoration: underline !important
    }
    .mscom-navitem>div.subNavContainer {
        position: relative;
        background-color: #fff
    }
    .mscom-navitem>div.subNavContainer>div {
        display: block !important;
        margin-left: 0
    }
    .mscom-header-nav-section {
        display: block !important
    }
    .subpageBody>div:first-of-type div.complianceSort {
        display: block !important
    }
    div.certPopup {
        position: fixed;
        left: 0 !important;
        right: 0 !important;
        top: 50px !important;
        width: 98% !important;
        margin: 0 auto
    }
}
@media(max-width:480px) {
    .showMobile {
        display: block
    }
    .hideMobile {
        display: none
    }
    div.slideshow-hero ul.slides li div.heroitem div.text {
        height: auto
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container {
        height: auto
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box {
        height: auto;
        width: 100%;
        padding-top: 20px;
        padding-bottom: 30px
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box ul {
        margin-top: 0
    }
    .subpageBody>div:first-of-type {
        padding-right: 25px;
        padding-left: 25px
    }
    .subpageBody>div:nth-of-type(2) {
        width: 100% !important
    }
    .subpageBody>div:nth-of-type(2) a.withArrow {
        width: 100%
    }
    .subpageBody>div:first-of-type {
        width: 100% !important
    }
    .light-foreground .icon-left,
    .light-foreground .icon-right {
        display: none
    }
    div.slideshow-hero ul.slides li div.heroitem div.text div.text-container div.box p {
        font-size: 15px
    }
    .videoWrapper {
        margin-left: 25px;
        margin-right: 25px
    }
    div.certImageContainer div.certImageGray>div,
    div.certImageContainer div.certImageColor>div {
        text-align: center
    }
    div.certDescriptionArea div.certDescription>div>div:first-of-type,
    div.certDescriptionArea div.certDescription>div>div:nth-of-type(2) {
        margin-left: 25px !important;
        margin-right: 25px
    }
    .features {
        width: 245px !important
    }
    #Services {
        margin-bottom: 0 !important
    }
    .features>div,
    #Services>div,
    #Services2>div {
        width: 100% !important
    }
    #Services p,
    #Services2 p {
        width: auto
    }
    #Services p a.withArrow,
    #Services2 p a.withArrow {
        display: block !important;
        margin-bottom: 15px
    }
    .features>div>div.row-fluid>div:nth-of-type(2) {
        float: none;
        display: inline-block
    }
    .features>div>div.row-fluid>div:first-of-type {
        float: none
    }
    .whatsNew h1 {
        font-size: 35px;
        margin-left: 25px;
        margin-right: 25px;
        margin-bottom: 10px
    }
    .whatsNew p {
        font-size: 15px;
        margin-left: 25px;
        margin-right: 25px;
        padding-right: 0
    }
    .whatsNew a {
        margin-left: 25px;
        margin-right: 25px;
        display: block;
        margin-bottom: 15px
    }
    .whatsNew p a {
        margin-left: 0;
        margin-right: 0;
        display: inline;
        margin-bottom: 0
    }
    #Services a.moreLink,
    #Services2 a.moreLink {
        display: inline-block;
        margin-bottom: 15px
    }
    .scenarios>div:first-of-type {
        margin-bottom: 15px
    }
    .compliantServices>div {
        display: inline-block !important;
        float: none !important;
        max-width: 200px !important
    }
    .compliantServices>div:first-of-type {
        margin-left: 25px !important
    }
    .compliantServices>div:nth-of-type(2) {
        margin-right: 25px !important
    }
    .mobileHome {
        display: block
    }
    .mscom-nav .mscom-nav-container {
        background-color: #fff
    }
    div#trusted-cloud-nav {
        position: absolute;
        z-index: 1000;
        background-color: #fff;
        width: 300px;
        right: -300px;
        bottom: 0;
        top: 135px;
        overflow-y: scroll;
        box-shadow: 3px 3px 15px #a1a1a1
    }
    div#trusted-cloud-nav>ul {
        margin: 0
    }
    div#trusted-cloud-nav>ul li a {
        color: #000
    }
    div#trusted-cloud-nav>ul>li>a {
        line-height: 2.5em
    }
    div#trusted-cloud-nav>ul>li>a:hover {
        background-color: #a1a1a1
    }
    div#trusted-cloud-nav>ul>li ul li a:hover {
        text-decoration: underline !important
    }
    .mscom-navitem>div.subNavContainer {
        position: relative;
        background-color: #fff
    }
    .mscom-navitem>div.subNavContainer>div {
        display: block !important;
        margin-left: 0
    }
    .mscom-header-nav-section {
        display: block !important
    }
    .subpageBody>div:first-of-type div.row-fluid div.mscom-accordion>ul {
        position: relative
    }
    .subpageBody>div:first-of-type div.row-fluid li.mscom-accordion-item:first-of-type {
        float: none;
        top: 0;
        right: 0;
        position: absolute
    }
    .subpageBody>div:first-of-type div.row-fluid li.mscom-accordion-item:nth-of-type(2) {
        padding-top: 40px
    }
    .subpageBody>div:nth-of-type(2) .compliantServices>div {
        width: 90%;
        max-width: 90% !important;
        margin-left: auto !important;
        margin-right: auto !important;
        display: block !important
    }
    div.certPopup {
        position: fixed;
        left: 0 !important;
        right: 0 !important;
        top: 50px !important;
        width: 98% !important;
        margin: 0 auto
    }
    .subpageBody>div:first-of-type div.complianceSort {
        display: block !important
    }
}